...
const ServiceSlider = () => {
  ...
  return (
    <section
      ...
      className="px-2 md:px-8 overflow-hidden"
    >
      <Slider {...settings}>
        {sliderItems.map((item, index) => (
          <article key={item.id} className="px-1 md:px-2">
            <div className="h-52 sm:h-64 md:h-80 rounded-xl overflow-hidden bg-white shadow-sm">
              {item.image && (
                <Image
                  src={item.image}
                  alt={item.title}
