...
const generateAccessToken = (userId, role = "user") =>{
    return jwt.sign({id:userId, role},process.env.JWT_ACCESS_SECRET,ACCESS_TOKEN_EXPIRY);
}

const generateRefreshToken = (userId, role = "user") => {
    return jwt.sign({id:userId, role},process.env.JWT_REFRESH_SECRET,REFRESH_TOKEN_EXPIRY);
}
