...
import RichTextEditor from "@/app/admin/components/RichTextField.jsx";
...
const filterNonEmpty = (arr = []) =>
  arr.filter((item) => (typeof item === "string" ? item.trim() !== "" : item));
...
const CATEGORIES = [
  ...
  { value: "JobGuarantee", label: "Job Guarantee" },
];
...
function SectionHeader({ title }) {
  return (
    <h2 className="font-semibold text-xl mb-3 border-b pb-1 text-gray-800">
      {title}
    </h2>
  );
}
...
function InputField({ label, ...props }) {
  return (
    <div>
      ...
      <input {...props} className="w-full border p-2 rounded-md" />
    </div>
  );
}
...
export default function CoursesPage() {
  ...
  const [careervidyabenifit, setCareervidyabenifit] = useState([
    { title: "", description: "" },
  ]);
  ...
  // ---------- ✅ NEW: COURSE TESTIMONIALS ----------
  const [courseTestimonials, setCourseTestimonials] = useState([
    {
      name: "", course: "", university: "", review: "",
      rating: 5, year: "", image: null,
    },
  ]);
  ...
  const removeNestedArrayItem = (setter, index, key, innerIndex) => {
    setter((prev) => {
      const updated = [...prev];
      updated[index][key] = updated[index][key].filter((_, i) => i !== innerIndex);
      if (updated[index][key].length === 0) updated[index][key].push("");
      return updated;
    });
  };
  ...
  const handleSpecializationChange = (i, val) =>
    handleArrayChange(setSpecializations, i, val);

  const handleGoodThingChange = (i, val) =>
    handleArrayChange(setGoodThings, i, val);
  ...
  const handleDetailedFeesTableChange = (outerIndex, tableIndex, key, value) => {
    setDetailedFees((prev) => {
      const updated = [...prev];
      updated[outerIndex].table[tableIndex] = {
        ...updated[outerIndex].table[tableIndex],
        [key]: value,
      };
      return updated;
  ...
  const addDetailedFeesTableItem = (index) => {
    setDetailedFees((prev) => {
      const updated = [...prev];
      updated[index].table.push({
        universityName: "", courseFees: "", detailedFeeStructure: "",
      });
      return updated;
    });
  };
  ...
  const removeDetailedFeesTableItem = (outerIndex, tableIndex) => {
    setDetailedFees((prev) => {
      const updated = [...prev];
      updated[outerIndex].table = updated[outerIndex].table.filter(
        (_, i) => i !== tableIndex
      );
      return updated;
    });
  ...
  const handleOnlineCourseWorthItTopicChange = (i, key, value) => {
    setOnlineCourseWorthIt((prev) => {
      const updatedTopics = [...prev.topics];
      updatedTopics[i] = { ...updatedTopics[i], [key]: value };
      return { ...prev, topics: updatedTopics };
    });
  };
  ...
  // ==================================================
  // ✅ SPECIALIZATION DETAILS HANDLERS
  // ==================================================
  const handleSpecDetailChange = (i, key, value) => {
  ...
  const handleSpecCareerRoleChange = (specIdx, roleIdx, key, value) => {
    setSpecializationDetails((prev) => {
      const updated = [...prev];
      updated[specIdx].careerRoles[roleIdx] = {
        ...updated[specIdx].careerRoles[roleIdx],
        [key]: value,
      };
      return updated;
  ...
  const addSpecCareerRole = (specIdx) => {
    setSpecializationDetails((prev) => {
      const updated = [...prev];
      updated[specIdx].careerRoles.push({ role: "", salary: "" });
      return updated;
    });
  };
  ...
  const removeSpecCareerRole = (specIdx, roleIdx) => {
    setSpecializationDetails((prev) => {
      const updated = [...prev];
      updated[specIdx].careerRoles = updated[specIdx].careerRoles.filter(
        (_, i) => i !== roleIdx
      );
      return updated;
    });
  ...
  // ==================================================
  const handleTestimonialChange = (i, key, value) => {
  ...
  const handleTestimonialImageChange = (i, file) => {
    setCourseTestimonials((prev) => {
      const updated = [...prev];
  ...
  const handlePlacementChange = (key, value) => {
    setPlacementSupport((prev) => ({ ...prev, [key]: value }));
  };

  const handlePlacementStatChange = (key, value) => {
  ...
  const buildPayload = () => {
    ...
    // Specializations
    payload.append("specializations", JSON.stringify(filterNonEmpty(specializations)));
    ...
    payload.append(
      "overview",
      JSON.stringify(overview.map(({ image, ...rest }) => rest))
    );
    overview.forEach((item) => {
    ...
    payload.append(
      "whyChooseUs",
      JSON.stringify(whyChooseUs.map(({ image, ...rest }) => rest))
    );
    whyChooseUs.forEach((item) => {
    ...
    // Simple arrays
    payload.append("goodThings", JSON.stringify(filterNonEmpty(goodThings)));
    payload.append(
      "keyHighlights",
      JSON.stringify(keyHighlights.filter((item) => item.heading))
    );
    ...
    payload.append(
      ...
      JSON.stringify(
        syllabus
          .filter((sem) => sem.semester)
          .map((sem) => ({ ...sem, subjects: filterNonEmpty(sem.subjects) }))
      )
    );
    ...
    payload.append(
      ...
      JSON.stringify(
        offeredCourses
          .filter((item) => item.heading)
          .map((item) => ({ ...item, points: filterNonEmpty(item.points) }))
      )
    );
    ...
    payload.append(
      "onlineEligibility",
      JSON.stringify(onlineEligibility.filter((item) => item.heading))
    );
    ...
    payload.append(
      ...
      JSON.stringify({
        ...
        tenureMonths: emiOptions.tenureMonths
          ? emiOptions.tenureMonths.split(",").map((n) => Number(n.trim())).filter(Boolean)
          : [],
      })
    );
    ...
    payload.append(
      ...
      JSON.stringify({
        description: onlineCourseWorthIt.description,
        topics: onlineCourseWorthIt.topics.filter((t) => t.subHeading),
      })
    );
    if (onlineCourseWorthIt.image)
      payload.append("onlineCourseWorthItImage", onlineCourseWorthIt.image);
    ...
    payload.append(
      "jobOpportunities",
      JSON.stringify(jobOpportunities.filter((item) => item.heading))
    );

    // Top Recruiters
    payload.append(
      "topRecruiters",
      JSON.stringify(topRecruiters.filter((item) => item.companyName))
    );

    // Placement Support
    payload.append("placementSupport", JSON.stringify(placementSupport));

    // Course Testimonials
    const testimonialTextData = courseTestimonials
      .filter((t) => t.name && t.review)
      .map(({ image, ...rest }) => rest);
    payload.append("courseTestimonials", JSON.stringify(testimonialTextData));
    courseTestimonials.forEach((t) => {
      if (t.image) payload.append("courseTestimonialImages", t.image);
    });

    // Career Vidya Benefits
    payload.append(
      "Careervidyabenifit",
      JSON.stringify(careervidyabenifit.filter((item) => item.title))
    );

    // FAQs
    payload.append(
      "faqs",
      JSON.stringify(faqs.filter((item) => item.question && item.answer))
    );

    // Universities (with course-specific data)
    payload.append("universities", JSON.stringify(universities));
    ...
    <main className="min-h-screen bg-gray-50 p-8">
      ...
      >
        ...
        <section className="grid md:grid-cols-2 gap-6">
          ...
          <InputField label="Name" type="text" name="name"
            placeholder="Course name" value={formData.name}
            onChange={handleChange} required />
          <InputField label="Duration" type="text" name="duration"
            placeholder="3 Years" value={formData.duration}
            onChange={handleChange} required />
          <InputField label="Tag" type="text" name="tag"
            placeholder="Optional tag" value={formData.tag}
            onChange={handleChange} />
          <div>
            <label>Course Logo</label>
        ...
        {/* ========== UNIVERSITIES ========== */}
        <section className="border-2 border-blue-200 p-4 rounded-md bg-blue-50/30">
          <SectionHeader title="🎯 Universities for this Course" />
          <p className="text-sm text-gray-600 mb-4">
            Select universities from your database. Har selected university ke liye
        ...
        {/* ========== SPECIALIZATIONS ========== */}
        <section>
          <SectionHeader title="Specializations (Simple List)" />
          {specializations.map((spec, i) => (
            <div key={i} className="flex gap-2 mb-2">
              <InputField type="text" placeholder="Specialization name"
                value={spec}
                onChange={(e) => handleSpecializationChange(i, e.target.value)} />
              {i > 0 && <RemoveButton onClick={() => removeField(setSpecializations, i)} />}
            </div>
          ))}
          <AddButton onClick={() => addField(setSpecializations, "")}>
            + Add Specialization
          </AddButton>
        </section>
        ...
        {/* ========== SPECIALIZATION DETAILS ========== */}
        <section>
          <SectionHeader title="Specialization Details (Advanced)" />
          {specializationDetails.map((spec, i) => (
            <div key={i} className="border p-4 rounded-md mb-4 bg-gray-50">
              <div className="flex gap-2 mb-2">
                <InputField type="text" placeholder="Specialization name"
                  value={spec.name}
                  onChange={(e) => handleSpecDetailChange(i, "name", e.target.value)} />
                {i > 0 && <RemoveButton onClick={() => removeField(setSpecializationDetails, i)} />}
              </div>
              <div className="mb-3">
                <RichTextEditor
                  value={spec.description}
                  onChange={(html) => handleSpecDetailChange(i, "description", html)}
                  placeholder="Description"
                  minHeight={80}
                />
              </div>

              <h4 className="font-medium text-sm my-2">Career Roles:</h4>
              {spec.careerRoles.map((role, j) => (
                <div key={j} className="grid grid-cols-2 gap-2 mb-2">
                  <InputField type="text" placeholder="Role (e.g., Data Analyst)"
                    value={role.role}
                    onChange={(e) => handleSpecCareerRoleChange(i, j, "role", e.target.value)} />
                  <div className="flex gap-2">
                    <InputField type="text" placeholder="Salary (₹3-6 LPA)"
                      value={role.salary}
                      onChange={(e) => handleSpecCareerRoleChange(i, j, "salary", e.target.value)} />
                    {j > 0 && (
                      <RemoveButton onClick={() => removeSpecCareerRole(i, j)} label="-" />
                    )}
                  </div>
                </div>
              ))}
              <AddButton onClick={() => addSpecCareerRole(i)}>+ Add Career Role</AddButton>
            </div>
          ))}
          <AddButton
            onClick={() =>
              addField(setSpecializationDetails, {
                name: "",
                description: "",
                careerRoles: [{ role: "", salary: "" }],
              })
        ...
        <section>
          ...
          {overview.map((item, i) => (
            <div key={i} className="grid md:grid-cols-2 gap-2 mb-4 border p-3 rounded-md">
              <InputField type="text" placeholder="Heading"
                value={item.heading}
                onChange={(e) => handleDynamicChange(setOverview, i, "heading", e.target.value)} />
              <InputField type="text" placeholder="Video Link"
                value={item.videoLink}
                onChange={(e) => handleDynamicChange(setOverview, i, "videoLink", e.target.value)} />
              <div className="md:col-span-2">
                <RichTextEditor
                  value={item.description}
                  onChange={(html) => handleDynamicChange(setOverview, i, "description", html)}
                  placeholder="Description"
                  minHeight={100}
                />
              </div>
              <input type="file" accept="image/*"
                onChange={(e) => handleDynamicChange(setOverview, i, "image", e.target.files[0])}
                className="border p-2 rounded-md md:col-span-2" />
            </div>
          ))}
          <AddButton
        ...
        {/* ========== SYLLABUS PDF ========== */}
        <div className="mb-4">
          <label className="block text-sm font-semibold mb-1">Syllabus PDF</label>
        ...
        <section>
          ...
          {whyChooseUs.map((item, i) => (
            <div key={i} className="flex flex-col md:flex-row gap-2 mb-4 border p-3 rounded-md">
              <input type="file" accept="image/*"
                onChange={(e) => handleDynamicChange(setWhyChooseUs, i, "image", e.target.files[0])}
                className="border p-2 rounded-md" />
              <div className="w-full">
                <RichTextEditor
                  value={item.description}
                  onChange={(html) => handleDynamicChange(setWhyChooseUs, i, "description", html)}
                  placeholder="Description"
                  minHeight={100}
                />
              </div>
        ...
        {/* ========== GOOD THINGS ========== */}
        <section>
          <SectionHeader title="Program And Highlight" />
          {goodThings.map((g, i) => (
            <div key={i} className="flex gap-2 mb-2">
              <InputField type="text" placeholder="Point about the course"
                value={g} onChange={(e) => handleGoodThingChange(i, e.target.value)} />
              {i > 0 && <RemoveButton onClick={() => removeField(setGoodThings, i)} />}
            </div>
          ))}
          <AddButton onClick={() => addField(setGoodThings, "")}>+ Add</AddButton>
        </section>
        ...
        <section>
          ...
          {keyHighlights.map((item, i) => (
            <div key={i} className="grid md:grid-cols-3 gap-2 mb-4 border p-3 rounded-md">
              ...
              <InputField type="text" placeholder="Sub Heading"
                value={item.subHeading}
                onChange={(e) => handleDynamicChange(setKeyHighlights, i, "subHeading", e.target.value)} />
              <div className="md:col-span-3">
                <RichTextEditor
                  value={item.description}
                  onChange={(html) => handleDynamicChange(setKeyHighlights, i, "description", html)}
                  placeholder="Description"
                  minHeight={100}
                />
              </div>
          ...
          <AddButton
            onClick={() =>
              addField(setKeyHighlights, { heading: "", subHeading: "", description: "" })
            }
          >
            + Add
          </AddButton>
        ...
        <section>
          ...
          {syllabus.map((sem, i) => (
            <div key={i} className="border p-4 rounded-md mb-4">
              <div className="flex gap-2 mb-2">
                <InputField type="text" placeholder="Semester"
                  value={sem.semester}
                  onChange={(e) => handleDynamicChange(setSyllabus, i, "semester", e.target.value)} />
                {i > 0 && <RemoveButton onClick={() => removeField(setSyllabus, i)} />}
              </div>
              <h4 className="font-medium text-sm my-2">Subjects:</h4>
              {sem.subjects.map((sub, j) => (
                <div key={j} className="flex gap-2 mb-1">
                  <InputField type="text" placeholder={`Subject ${j + 1}`}
                    value={sub}
                    onChange={(e) => handleSubjectChange(i, j, e.target.value)} />
                  {j > 0 && (
                    <RemoveButton
                      onClick={() => removeNestedArrayItem(setSyllabus, i, "subjects", j)}
                      label="-" />
                  )}
                </div>
              ))}
              <AddButton onClick={() => addNestedArrayItem(setSyllabus, i, "subjects")}>
                + Add Subject
              </AddButton>
            </div>
          ))}
        ...
        {/* ========== OFFERED COURSES ========== */}
        <section>
          <SectionHeader title="Offered Courses" />
          {offeredCourses.map((item, i) => (
            <div key={i} className="border p-4 rounded-md mb-4">
              <div className="flex gap-2 mb-2">
                <InputField type="text" placeholder="Heading"
                  value={item.heading}
                  onChange={(e) => handleDynamicChange(setOfferedCourses, i, "heading", e.target.value)} />
                {i > 0 && <RemoveButton onClick={() => removeField(setOfferedCourses, i)} />}
              </div>
              <h4 className="font-medium text-sm my-2">Points:</h4>
              {item.points.map((point, j) => (
                <div key={j} className="flex gap-2 mb-1">
                  <InputField type="text" placeholder={`Point ${j + 1}`}
                    value={point}
                    onChange={(e) =>
                      handleNestedDynamicChange(setOfferedCourses, i, "points", j, e.target.value)
                    } />
                  {j > 0 && (
                    <RemoveButton
                      onClick={() => removeNestedArrayItem(setOfferedCourses, i, "points", j)}
              ...
              <AddButton onClick={() => addNestedArrayItem(setOfferedCourses, i, "points")}>
                + Add Point
              </AddButton>
            </div>
          ))}
        ...
        {/* ========== ONLINE ELIGIBILITY ========== */}
        <section>
          <SectionHeader title="Online Course Eligibility" />
          {onlineEligibility.map((item, i) => (
            <div key={i} className="border p-4 rounded-md mb-4">
              <div className="grid md:grid-cols-2 gap-2 mb-2">
                <InputField type="text" placeholder="Heading"
                  value={item.heading}
                  onChange={(e) => handleDynamicChange(setOnlineEligibility, i, "heading", e.target.value)} />
                <InputField type="text" placeholder="Sub Heading (Optional)"
                  value={item.subHeading}
                  onChange={(e) => handleDynamicChange(setOnlineEligibility, i, "subHeading", e.target.value)} />
                <div className="md:col-span-2">
                  <RichTextEditor
                    value={item.description}
                    onChange={(html) => handleDynamicChange(setOnlineEligibility, i, "description", html)}
                    placeholder="Description"
                    minHeight={100}
                  />
                </div>
                <div className="md:col-span-2">
                  <RichTextEditor
                    value={item.subDescription}
                    onChange={(html) => handleDynamicChange(setOnlineEligibility, i, "subDescription", html)}
                    placeholder="Sub Description (Optional)"
                    minHeight={80}
                  />
                </div>
              ...
              {i > 0 && (
                <RemoveButton onClick={() => removeField(setOnlineEligibility, i)}
                  label="✕ Remove" />
              )}
            </div>
          ))}
        ...
        <section>
          ...
          {admissionProcess.map((step, i) => (
            <div key={i} className="border p-4 rounded-md mb-3 bg-slate-50">
              <div className="flex gap-2 mb-2 items-center">
                <span className="bg-blue-600 text-white w-8 h-8 rounded-full flex items-center justify-center font-bold">
                  {i + 1}
                </span>
                <InputField type="text" placeholder="Title (e.g., Check Eligibility)"
                  value={step.title}
                  onChange={(e) => handleDynamicChange(setAdmissionProcess, i, "title", e.target.value)} />
                {i > 0 && (
                  <RemoveButton onClick={() => removeField(setAdmissionProcess, i)} />
                )}
              </div>
              <RichTextEditor
                value={step.description}
                onChange={(html) => handleDynamicChange(setAdmissionProcess, i, "description", html)}
                placeholder="Description"
                minHeight={80}
              />
            </div>
          ...
          <AddButton
            onClick={() =>
              addField(setAdmissionProcess, {
                step: admissionProcess.length + 1, title: "", description: "",
              })
            }
          >
            + Add Admission Step
        ...
        {/* ========== FEES STRUCTURE ========== */}
        <section>
          <SectionHeader title="Fees Structure (Sidebar)" />
          {feeStructureSidebar.map((item, i) => (
            <div key={i} className="border p-4 rounded-md mb-4">
              <div className="flex gap-2 mb-2">
                <InputField type="text" placeholder="Heading"
                  value={item.heading}
                  onChange={(e) => handleDynamicChange(setFeeStructureSidebar, i, "heading", e.target.value)} />
                {i > 0 && <RemoveButton onClick={() => removeField(setFeeStructureSidebar, i)} />}
              </div>
              <h4 className="font-medium text-sm my-2">Points:</h4>
              {item.points.map((point, j) => (
                <div key={j} className="flex gap-2 mb-1">
                  <InputField type="text" placeholder={`Point ${j + 1}`}
                    value={point}
                    onChange={(e) =>
                      handleNestedDynamicChange(setFeeStructureSidebar, i, "points", j, e.target.value)
                    } />
                  {j > 0 && (
                    <RemoveButton
                      onClick={() => removeNestedArrayItem(setFeeStructureSidebar, i, "points", j)}
              ...
              <AddButton onClick={() => addNestedArrayItem(setFeeStructureSidebar, i, "points")}>
                + Add Point
              </AddButton>
            </div>
          ))}
        ...
        {/* ========== DETAILED FEES ========== */}
        <section>
          <SectionHeader title="Detailed Fees Table" />
          {detailedFees.map((item, i) => (
            <div key={i} className="border p-4 rounded-md mb-4">
              <div className="flex gap-2 mb-2">
                <InputField type="text" placeholder="Heading"
                  value={item.heading}
                  onChange={(e) => handleDynamicChange(setDetailedFees, i, "heading", e.target.value)} />
                {i > 0 && <RemoveButton onClick={() => removeField(setDetailedFees, i)} />}
              </div>
              <div className="mb-2">
                <RichTextEditor
                  value={item.description}
                  onChange={(html) => handleDynamicChange(setDetailedFees, i, "description", html)}
                  placeholder="Description"
                  minHeight={80}
                />
              </div>
              ...
              {item.table.map((row, j) => (
                <div key={j}
                  className="grid md:grid-cols-3 gap-2 mb-2 p-2 border rounded-md bg-gray-50">
                  <InputField type="text" placeholder="University Name"
                    value={row.universityName}
                    onChange={(e) =>
                      handleDetailedFeesTableChange(i, j, "universityName", e.target.value)
                    } />
                  <InputField type="text" placeholder="Course Fees"
                    value={row.courseFees}
                    onChange={(e) =>
                      handleDetailedFeesTableChange(i, j, "courseFees", e.target.value)
                    } />
                  <div className="flex gap-2">
                    <InputField type="text" placeholder="Details"
                      value={row.detailedFeeStructure}
                      onChange={(e) =>
                        handleDetailedFeesTableChange(i, j, "detailedFeeStructure", e.target.value)
                      } />
                    {j > 0 && (
                      <RemoveButton onClick={() => removeDetailedFeesTableItem(i, j)} label="-" />
                    )}
                  </div>
                </div>
              ))}
              <AddButton onClick={() => addDetailedFeesTableItem(i)}>
                + Add Table Row
              </AddButton>
            </div>
          ))}
        ...
        {/* ========== EMI OPTIONS ========== */}
        <section>
          <SectionHeader title="💳 EMI Options" />
          <div className="border p-4 rounded-md">
            ...
            {emiOptions.enabled && (
              <div className="grid md:grid-cols-3 gap-3">
                <InputField type="text" placeholder="Min Monthly (₹3,000)"
                  value={emiOptions.minMonthly}
                  onChange={(e) =>
                    setEmiOptions((prev) => ({ ...prev, minMonthly: e.target.value }))
                  } />
                <InputField type="text" placeholder="Max Monthly (₹5,000)"
                  value={emiOptions.maxMonthly}
                  onChange={(e) =>
                    setEmiOptions((prev) => ({ ...prev, maxMonthly: e.target.value }))
                  } />
                <InputField type="text" placeholder="Tenures (6,12,24)"
                  value={emiOptions.tenureMonths}
                  onChange={(e) =>
                    setEmiOptions((prev) => ({ ...prev, tenureMonths: e.target.value }))
                  } />
              </div>
            )}
          </div>
        ...
        <section>
          ...
          {scholarships.map((item, i) => (
            <div key={i} className="border p-4 rounded-md mb-3">
              <div className="flex gap-2 mb-2">
                <InputField type="text" placeholder="Title (Merit Scholarship)"
                  value={item.title}
                  onChange={(e) => handleDynamicChange(setScholarships, i, "title", e.target.value)} />
                <InputField type="text" placeholder="Discount (up to 30%)"
                  value={item.discount}
                  onChange={(e) => handleDynamicChange(setScholarships, i, "discount", e.target.value)} />
                {i > 0 && <RemoveButton onClick={() => removeField(setScholarships, i)} />}
              </div>
              <RichTextEditor
                value={item.description}
                onChange={(html) => handleDynamicChange(setScholarships, i, "description", html)}
                placeholder="Description"
                minHeight={80}
              />
            </div>
          ...
          <AddButton
            onClick={() =>
              addField(setScholarships, { title: "", discount: "", description: "" })
            }
          >
            + Add Scholarship
          </AddButton>
        ...
        {/* ========== ONLINE COURSE WORTH IT ========== */}
        <section>
          <SectionHeader title="Online Course Worth It?" />
          <div className="mb-4">
            <RichTextEditor
              value={onlineCourseWorthIt.description}
              onChange={(html) =>
                setOnlineCourseWorthIt((prev) => ({ ...prev, description: html }))
              }
              placeholder="Main Description"
              minHeight={120}
            />
          ...
          <input type="file" accept="image/*"
            onChange={(e) =>
              setOnlineCourseWorthIt((prev) => ({ ...prev, image: e.target.files[0] }))
            }
            className="border p-2 rounded-md w-full mb-4" />
          ...
          {onlineCourseWorthIt.topics.map((topic, i) => (
            <div key={i} className="border p-3 rounded-md mb-2 bg-gray-50">
              <InputField type="text" placeholder="Sub Heading"
                value={topic.subHeading}
                onChange={(e) =>
                  handleOnlineCourseWorthItTopicChange(i, "subHeading", e.target.value)
                } />
              <div className="mb-2 mt-2">
                <RichTextEditor
                  value={topic.description}
                  onChange={(html) =>
                    handleOnlineCourseWorthItTopicChange(i, "description", html)
                  }
                  placeholder="Description"
                  minHeight={80}
                />
          ...
          <AddButton onClick={addOnlineCourseWorthItTopic}>+ Add Topic</AddButton>
        </section>
            ...
            <div key={i} className="border p-4 rounded-md mb-4">
              <div className="grid md:grid-cols-2 gap-2 mb-2">
                <InputField type="text" placeholder="Heading"
                  value={item.heading}
                  onChange={(e) => handleDynamicChange(setJobOpportunities, i, "heading", e.target.value)} />
                <InputField type="text" placeholder="Job Post"
                  value={item.jobPost}
                  onChange={(e) => handleDynamicChange(setJobOpportunities, i, "jobPost", e.target.value)} />
                <div className="md:col-span-2">
                  <RichTextEditor
                    value={item.description}
                    onChange={(html) => handleDynamicChange(setJobOpportunities, i, "description", html)}
                    placeholder="Description"
                    minHeight={80}
                  />
                </div>
              ...
              {i > 0 && (
                <RemoveButton onClick={() => removeField(setJobOpportunities, i)}
                  label="✕ Remove" />
              )}
            </div>
          ))}
        ...
        <section>
          ...
          {topRecruiters.map((item, i) => (
            <div key={i} className="flex gap-2 mb-2">
              ...
              <InputField type="text" placeholder="Package Offered"
                value={item.packageOffered}
                onChange={(e) =>
                  handleDynamicChange(setTopRecruiters, i, "packageOffered", e.target.value)
                } />
              {i > 0 && <RemoveButton onClick={() => removeField(setTopRecruiters, i)} />}
            </div>
          ))}
          <AddButton
            onClick={() =>
              addField(setTopRecruiters, { companyName: "", packageOffered: "" })
            }
          >
            + Add Recruiter
          </AddButton>
        ...
        {/* ========== PLACEMENT SUPPORT ========== */}
        <section>
          <SectionHeader title="💼 Placement Support" />
          <div className="border p-4 rounded-md">
            <RichTextEditor
              value={placementSupport.description}
              onChange={(html) => handlePlacementChange("description", html)}
              placeholder="Description"
              minHeight={100}
            />

            <h4 className="font-medium text-sm my-3">Placement Stats:</h4>
            <div className="grid md:grid-cols-3 gap-3">
              <InputField type="text" placeholder="Placement Rate (85%)"
                value={placementSupport.stats.placementRate}
                onChange={(e) => handlePlacementStatChange("placementRate", e.target.value)} />
              <InputField type="text" placeholder="Avg Package (₹4.5 LPA)"
                value={placementSupport.stats.avgPackage}
                onChange={(e) => handlePlacementStatChange("avgPackage", e.target.value)} />
              <InputField type="text" placeholder="Highest Package (₹12 LPA)"
                value={placementSupport.stats.highestPackage}
                onChange={(e) => handlePlacementStatChange("highestPackage", e.target.value)} />
            </div>
          </div>
        </section>
        ...
        {/* ========== COURSE TESTIMONIALS ========== */}
        <section>
          <SectionHeader title="⭐ Course Testimonials" />
          {courseTestimonials.map((item, i) => (
            <div key={i} className="border p-4 rounded-md mb-4">
              <div className="flex gap-2 mb-2">
                <InputField type="text" placeholder="Student Name"
                  value={item.name}
                  onChange={(e) => handleTestimonialChange(i, "name", e.target.value)} />
                {i > 0 && <RemoveButton onClick={() => removeField(setCourseTestimonials, i)} />}
              </div>
              <div className="grid md:grid-cols-3 gap-2 mb-2">
                <InputField type="text" placeholder="Course (Online BCA)"
                  value={item.course}
                  onChange={(e) => handleTestimonialChange(i, "course", e.target.value)} />
                <InputField type="text" placeholder="University (Amity)"
                  value={item.university}
                  onChange={(e) => handleTestimonialChange(i, "university", e.target.value)} />
                <InputField type="number" placeholder="Rating (1-5)" min={1} max={5}
                  value={item.rating}
                  onChange={(e) => handleTestimonialChange(i, "rating", Number(e.target.value))} />
              </div>
              <div className="grid md:grid-cols-2 gap-2 mb-2">
                <InputField type="text" placeholder="Year (2024)"
                  value={item.year}
                  onChange={(e) => handleTestimonialChange(i, "year", e.target.value)} />
                <input type="file" accept="image/*"
                  onChange={(e) => handleTestimonialImageChange(i, e.target.files[0])}
                  className="border p-2 rounded-md" />
              </div>
              <RichTextEditor
                value={item.review}
                onChange={(html) => handleTestimonialChange(i, "review", html)}
                placeholder="Review"
                minHeight={80}
              />
            </div>
          ...
          <AddButton
            onClick={() =>
              addField(setCourseTestimonials, {
                name: "", course: "", university: "", review: "",
                rating: 5, year: "", image: null,
              })
            }
        ...
        {/* ========== CAREER VIDYA BENEFITS ========== */}
        <section>
          <SectionHeader title="Career Vidya Benefits" />
          {careervidyabenifit.map((item, i) => (
            <div key={i} className="border p-4 rounded-md mb-4">
              <div className="flex gap-2 mb-2">
                <InputField type="text" placeholder="Title"
                  value={item.title}
                  onChange={(e) =>
                    handleDynamicChange(setCareervidyabenifit, i, "title", e.target.value)
                  } />
                {i > 0 && <RemoveButton onClick={() => removeField(setCareervidyabenifit, i)} />}
              </div>
              <RichTextEditor
                value={item.description}
                onChange={(html) =>
                  handleDynamicChange(setCareervidyabenifit, i, "description", html)
                }
                placeholder="Description"
                minHeight={80}
              />
          ...
          <AddButton
            onClick={() =>
              addField(setCareervidyabenifit, { title: "", description: "" })
            }
          >
            + Add Benefit
          </AddButton>
        ...
        <section>
          ...
          {faqs.map((item, i) => (
            <div key={i} className="border p-4 rounded-md mb-4">
              <div className="flex gap-2 mb-2">
                <InputField type="text" placeholder="Question"
                  value={item.question}
                  onChange={(e) => handleDynamicChange(setFaqs, i, "question", e.target.value)} />
                {i > 0 && <RemoveButton onClick={() => removeField(setFaqs, i)} />}
              </div>
              <RichTextEditor
                value={item.answer}
                onChange={(html) => handleDynamicChange(setFaqs, i, "answer", html)}
                placeholder="Answer"
                minHeight={100}
              />
            </div>
          ))}
          <AddButton onClick={() => addField(setFaqs, { question: "", answer: "" })}>
            + Add FAQ
          </AddButton>
        </section>
        ...
        <button type="submit" disabled={loading}
          className="bg-blue-600 text-white w-full py-3 rounded-md hover:bg-blue-700 disabled:opacity-50 transition font-semibold">
          {loading ? "Saving..." : "Create Course"}
        </button>
      </form>
      ...
      <ul className="space-y-2">
        {courses.map((course) => (
          <li key={course._id}
            className="bg-white p-3 rounded-md shadow-sm border border-gray-100">
            {course.name} — <strong>{course.category}</strong> (Slug: {course.slug})
          </li>
        ))}
      </ul>
    </main>
