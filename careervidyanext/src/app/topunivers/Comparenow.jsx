...
/* ================= MAIN COMPONENT ================= */
// ✅ isLoggedIn prop add kiya — agar true hai to form skip karke seedha result page pe redirect
const Signup = ({ onClose, selectedUnis = [], isLoggedIn = false }) => {
  ...
  // ✅ Agar user already logged in hai, to form dikhane ki zaroorat nahi —
  // seedha result page pe bhej do aur modal close kar do.
  useEffect(() => {
    if (isLoggedIn) {
      const ids = selectedUnis.map((u) => u._id).join(",");
  ...
  useEffect(() => {
    if (isLoggedIn) return; // logged-in user ke liye ye data load karne ki zaroorat nahi
    const fetchStates = async () => {
      try {
        const res = await api.get("/api/v1/states");
        setStates(res.data.states || []);
  ...
  // ✅ Logged-in user ke liye kuch bhi render nahi karna — redirect useEffect me ho raha hai
  if (isLoggedIn) {
    return null;
  }
