...
//       {/* SEO Improvement: Heading mein Course Name add kiya */}
...
//                 {/* SEO Improvement: Feature word ki jagah Benefit context diya aur h3 tag use kiya */}
//                 <h3 className="text-[13px] font-bold text-blue-600 uppercase tracking-widest">
//                    Key Advantage {i + 1}
//                 </h3>
//                 <svg width="20" height="20" viewBox="0 0 24 24" fill="none" className="text-blue-500 opacity-20 group-hover:opacity-100 transition-opacity">
//                    <path d="M20 6L9 17L4 12" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
//                 </svg>
//               </div>

//               {/* Text-left rakha hai extra spacing hatane ke liye (apki pichli request ke hisab se) */}
//               <p className="text-[#002147] text-[16px] leading-relaxed font-normal text-left">
//                 {g}
//               </p>
//             </div>
//           </div>
...
export default function CourseDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchCourse = async () => {
      try {
        // API call fix: checking if api has .get or it is a direct instance
        const res = await api.get(`/api/v1/course/slug/${slug}`);
        setCourse(res.data.course);
      } catch (err) {
        console.error("❌ Error fetching course:", err);
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-white">
        ...
        {(course.feeStructureSidebar?.length > 0 || course.detailedFees?.length > 0) && (
          <FeeStructure courseTitle={course.name} feeStructureSidebar={course.feeStructureSidebar} detailedFees={course.detailedFees} />
        )}
