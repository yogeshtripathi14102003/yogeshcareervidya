...
const CounselorDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex">
      {/* ================= SIDEBAR — navy blue theme, same as rest of the app ================= */}
      <aside
        className={`
          cv-sidebar fixed md:static inset-y-0 left-0 z-40
          w-64 text-white
          transform transition-transform duration-300
          ${sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}
        `}
      >
        <style>{`
          ...
          .cv-side-link:hover:not(:disabled) {
            background: rgba(255,255,255,0.07);
            color: #ffffff;
          }
          .cv-side-link:disabled {
            color: #5b6a95;
          }
          .cv-side-icon { color: #7dd3fc; }
          .cv-side-link:disabled .cv-side-icon { color: #45568a; }
        `}</style>

        {/* LOGO */}
        <div className="p-5 border-b border-white/10 text-xl font-bold">
          Counselor Panel
        </div>
        ...
        <div className="p-4 border-b border-white/10">
          <p className="font-semibold">{user.name}</p>
          <p className="text-xs" style={{ color: "#9db1e8" }}>{user.email}</p>
          <div
            className={`mt-2 inline-block px-2 py-0.5 rounded text-[10px] uppercase font-bold ${
          ...
          {SIDEBAR_LINKS.map((item) => {
            ...
            return (
              <button
                ...
                onClick={() => canAccess && router.push(item.path)}
                disabled={!canAccess}
                className={`cv-side-link w-full flex items-center gap-3 px-4 py-2 rounded-lg text-left transition-all
                  ${canAccess ? "cursor-pointer" : "cursor-not-allowed"}`}
                style={{ pointerEvents: canAccess ? "auto" : "none" }}
              >
                <Icon size={18} className="cv-side-icon" />
                <span className="flex-1">{item.name}</span>
                {!canAccess && <Lock size={14} style={{ color: "#7686b8" }} />}
              </button>
            );
          })}
        </nav>
