...
export default function BlogForm() {
  ...
  // ✅ NEW — rich text editor direct value ke saath set karne ke liye
  // (kyunki RichTextField event object nahi, seedha HTML string deta hai)
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Bio / Description" className="col-span-2">
              <RichTextField
                ...
                onChange={(val) => setValueAtPath(["author", "description"], val)}
          ...
          <div className="space-y-4 mt-4">
            {formData.content.map((block, i) => (
              <div key={i} className="border border-slate-200 rounded-lg p-4 bg-slate-50 relative space-y-3">
                ...
                {/* ── PARAGRAPH ── ✅ Rich Text Editor with link support */}
                {block.type === "paragraph" && (
                  <div className="grid grid-cols-2 gap-3">
                    <Field label="Text" className="col-span-2">
                      <RichTextField
                        value={block.text}
                        onChange={(val) => setValueAtPath(["content", i, "text"], val)}
                        placeholder="Paragraph content… select text and use the link icon to add a redirect link"
                      />
                    </Field>
                    <Field label="Alignment">
                      <select className={inp} value={block.align} onChange={(e) => handleChange(e, ["content", i, "align"])}>
                ...
                {["quote", "code"].includes(block.type) && (
                  <div className="space-y-3">
                    <Field label={block.type === "code" ? "Code" : "Quote Text"}>
                      {block.type === "quote" ? (
                        // ✅ Quote ke liye rich text (link support ke saath)
                        <RichTextField
                          value={block.text}
                          onChange={(val) => setValueAtPath(["content", i, "text"], val)}
                        ...
                        // Code ke liye plain textarea hi rehne dein — rich text formatting code ko todd degi
                        <textarea
                ...
                {/* ── LIST / NUMBERED LIST ── ✅ har item ab rich text (link ke saath) */}
                {["list", "number_list"].includes(block.type) && (
                  <Field label="List Items">
                    <div className="space-y-2">
                      {block.list_items.map((item, idx) => (
                        <div key={idx} className="flex gap-2 items-start">
                          ...
                          <div className="flex-1">
                            <RichTextField
                              value={item}
                              onChange={(val) => setValueAtPath(["content", i, "list_items", idx], val)}
                              placeholder={`Item ${idx + 1}… select text to add a link`}
                              minHeight="60px"
                            />
                          </div>
                          <button type="button" onClick={() => removeItem(["content", i, "list_items"], idx)} className="text-red-500 hover:text-red-700 shrink-0 pt-2">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      ))}
        ...
        {/* ── FAQs ── ✅ Answer ab rich text (link support ke saath) */}
        <Section title="FAQs">
          <div className="space-y-4">
            {formData.faqs.map((faq, i) => (
              <div key={i} className="border border-slate-200 rounded-lg p-4 bg-slate-50 space-y-3">
                ...
                <Field label="Answer">
                  <RichTextField
                    value={faq.answer}
                    onChange={(val) => setValueAtPath(["faqs", i, "answer"], val)}
                    placeholder="Enter answer… select text to add a link"
                  />
                </Field>
              </div>
            ))}
