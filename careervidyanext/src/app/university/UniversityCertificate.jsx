...
// ✅ FIX: was self-fetching /api/v1/university/slug/{slug} again — same data
// the parent (UniversityDetail) already fetched server-side and passed
// down. Removed useEffect/api call/loading/error state; now renders
// synchronously from the `data` prop, so this content is present in the
// SSR HTML instead of popping in after a client-side fetch.
//
// ✅ FIX: fallback heading/description were fixed literal strings
// ("Recognition" / "Earn a degree that is widely recognized around the
// globe.") with zero variation. If `recognition.recognitionHeading` is
// left empty in the admin panel for more than one university, every such
// page renders identical H2 text — a duplicate-H2 SEO flag. Fallbacks now
...
export default function UniversityCertificate({ data }) {
    ...
    const recognitionHeading = recognition.recognitionHeading || `${universityName} Recognition & Accreditation`;
    ...
    // Nothing to show for this university — don't render an empty section
    if (!recognition.recognitionHeading && !recognitionPoints.length && !certificateImage) return null;
            ...
            <div style={styles.contentContainer}>
                ...
                <div style={styles.imageColumn}>
                    {certificateImage ? (
                        ...
                        // needs either explicit dimensions or a fill parent —
                        // neither is safe to assume here without visual QA.
                        <img src={certificateImage} alt={`${universityName} Sample Certificate`} style={styles.certificateImage} />
                    ) : (
                        <div style={styles.noImagePlaceholder}>
                            Certificate Image Not Available
                        </div>
