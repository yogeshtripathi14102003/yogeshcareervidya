...
export default function TeamListingPage() {
    ...
    const openDetailPage = (mentor) => {
        ...
        if (mentorId) {
            router.push(`/teamexpand/${mentorId}`); 
        } else {
            ...
            <div className="bg-gray-50 min-h-screen py-10 px-4 md:px-8" id="mentor-list">
                <div className="max-w-7xl mx-auto">
                    <div className="flex flex-col md:flex-row gap-4 items-center justify-between mb-6">
                        <div className="flex-1 w-full md:w-auto">
                            <div className="relative">
                                <input className="w-full rounded-lg border border-gray-200 px-4 py-3 pl-11 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Search counsellors by name..." value={query} onChange={(e)=>setQuery(e.target.value)} />
                                <div className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"><Search className="w-5 h-5"/></div>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            <select value={sortBy} onChange={(e)=>setSortBy(e.target.value)} className="rounded-lg border border-gray-200 px-3 py-2 bg-white text-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                        <aside className="md:col-span-3 bg-white rounded-xl p-5 shadow-sm h-max">
                            ...
                            {loading ? (
                                ...
                                paged.map((m) => (
                                    <MentorCard 
                                        key={m._id || m.id} 
                                        mentor={m} 
                                        onBook={() => openBookModalHandler(m)} 
                                        onDetail={() => openDetailPage(m)} 
                                    />
                                ))
                            )}
                        </main>
