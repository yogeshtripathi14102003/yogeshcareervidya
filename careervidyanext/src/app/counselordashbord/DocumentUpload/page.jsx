...
"use client";
  ...
  // ── FIX: accept authUser as a parameter instead of reading it
  // from outer scope (it doesn't exist outside the component — that's
  // what caused "authUser is not defined").
  const getCounselorInfo = (authUser) => {
    return {
      id:   authUser?._id || null,
      name: authUser?.name || null,
    };
  ...
  const getCounselorParams = (authUser, extra = {}) => {
    const { name: counselorName } = getCounselorInfo(authUser);
    const params = { ...extra };
    if (counselorName) params.counselorName = counselorName;
    return params;
  };
  ...
  export default function CounselorPortal() {
    ...
    // ── FIX: only fetch once authUser is actually available,
    // otherwise counselorName param is sent empty on the first
    // render and you silently get either nothing or everyone's docs.
    useEffect(() => {
      if (authUser) {
        fetchAllStudents();
      }
    ...
    const fetchAllStudents = async () => {
      setListLoading(true);
      try {
        const params = getCounselorParams(authUser, { limit: 5000 });
        const res = await api.get("/api/v1/ad", { params });
        if (res.data?.success) setAllStudents(res.data.data || []);
      } catch (e) {
        console.error("Error fetching student list:", e);
