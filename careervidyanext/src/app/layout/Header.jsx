...
// ────── SHARED DATA: keep desktop mega-menu and mobile sidebar in sync ──────
// Edit this ONE list to update courses in both places at once.
const POPULAR_COURSES = [
  { href: "/course/online-mba-1",                            label: "Online MBA" },
  { href: "/course/online-mca",                              label: "Online MCA" },
  { href: "/course/bba-bachelor-of-business-administration", label: "Online BBA" },
  { href: "/course/online-bca-bachelor-of-computer-applications", label: "Online BCA" },
  { href: "/course/online-ba-bachelors-of-arts",             label: "Online BA" },
  { href: "/course/online-ma-masters-of-arts",               label: "Online MA" },
  { href: "/course/online-bcom-bachelors-of-commerce",       label: "Online B.Com" },
  { href: "/course/mcom-master-of-commerce",                 label: "M.Com" },
      { href: "/course/1-year-online-mba",                      label: "One Year Online MBA" },

  { href: "/course/btech-bachelors-of-technology",           label: "B.Tech" },

];

const TOP_UNIVERSITIES = [
  { href: "/university/amity-university-online",          label: "Amity University" },
  { href: "/university/lovely-professional-university",    label: "LPU Online" },
  { href: "/university/manipal-university-jaipur",         label: "Manipal University" },
  { href: "/university/chandigarh-university-online",      label: "Chandigarh University" },
  { href: "/university/srm-university",                    label: "SRM University" },
  { href: "/university/dy-patil-university-online-mumbai", label: "DY Patil University" },
  { href: "/university/chitkara-university",               label: "Chitkara University" },
  { href: "/university/kurukshetra-university-online",     label: "Kurukshetra University" },
  { href: "/university/sharda-university",                 label: "Sharda University" },
  { href: "/university/noida-international-university",    label: "Noida International University" },
  { href: "/university/gla-online",                        label: "GLA University Online" },
  { href: "/university/dr-a-p-j-abdul-kalam-university",   label: "Dr. APJ Abdul Kalam University" },
  { href: "/university/sikkim-manipal-university",         label: "Sikkim Manipal (SMU)" },
  { href: "/university/jaipur-national-university",        label: "Jaipur National University" },
  { href: "/university/jain-university",                   label: "Jain University" },
  { href: "/university/birla-institute-of-technology",     label: "Birla Institute of Technology" },
  { href: "/university/aks-university",                    label: "AKS University" },
  { href: "/university/era-university",                    label: "Era University" },
  { href: "/university/sanskriti-university",               label: "Sanskriti University" },
];
...
export default function Header() {
  ...
  const [user, setUser]                 = useState(null);
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const [coursesOpen, setCoursesOpen]   = useState(false); // mobile "Popular Courses" collapsible
  const [universitiesOpen, setUniversitiesOpen] = useState(false); // mobile "Top Universities" collapsible
        ...
        <div className="header-inner">
          ...
          <nav className="nav-center desktop-only">
            ...
            <div className="mega-menu-wrap relative group">
              ...
              {/* Widened from 720px → 820px to comfortably fit the 2-col course grid */}
              <div className="absolute top-full left-1/2 -translate-x-1/2 mt-4
                              w-[820px] bg-white rounded-2xl p-8 grid grid-cols-3 gap-8
                              shadow-[0_10px_40px_rgba(0,0,0,0.1)] border border-gray-100
                              opacity-0 invisible translate-y-3
                              group-hover:opacity-100 group-hover:visible group-hover:translate-y-0
                              transition-all duration-300 z-[999]">
                ...
                {/* ────── POPULAR COURSES — now a 2-col grid, 10 courses + View All ──────
                    SEO note: every course link below is a real <Link> in the server-rendered
                    markup (not injected after a loading state), so crawlers see all 10 hrefs
                    without needing to execute JS first. */}
                <div>
                  <h4 className="text-[15px] font-bold mb-4 text-[#002D62] border-b pb-2">Popular Courses</h4>
                  ...
                  <div className="mt-3 pt-3 border-t border-gray-100">
                    <Link href="/explore" className="text-sm font-bold text-[#04458b] hover:underline">
                      View All Courses →
                    </Link>
                  </div>
                </div>
        ...
        >
          ...
          >
            ...
            <nav className="mobile-nav-links">
              ...
              {/* ────── POPULAR COURSES — collapsible, mobile-only ──────
                  Fixes the audit finding: desktop mega-menu had course links,
                  mobile sidebar had none — so Google's mobile-first indexing
                  never saw them. These are real <Link> elements present in the
                  DOM at all times (just visually collapsed via CSS/state), so
                  they remain crawlable regardless of JS execution. */}
              <button
                type="button"
              ...
              {coursesOpen && (
                <div className="mobile-collapsible-panel">
                  {/* spacer-row: gives breathing room between the trigger
                      ("Explore Programs" row above) and the first course link,
                      so "One Year Online MBA" doesn't look glued to the row above it */}
              ...
              {/* ────── TOP UNIVERSITIES — collapsible, mobile-only ──────
                  Same pattern as Popular Courses above: was previously a single
                  link straight to /topunivers with no in-place preview, which
                  also made it look out of place sitting between two unrelated
                  rows in the sidebar. Now expands in place, consistent with
                  the desktop mega-menu and with the Popular Courses block. */}
              <button
                type="button"
                className="mobile-collapsible-trigger"
