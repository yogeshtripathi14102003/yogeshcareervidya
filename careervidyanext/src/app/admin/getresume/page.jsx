...
export default function ApplicationManagementPage() {
  ...
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedApp, setSelectedApp] = useState(null); // ✅ pura object rakhenge, sirf email nahi
  ...
  // Send Email Modal — ✅ ab pura app object bhejenge
  const handleSendEmailClick = (app) => {
    setSelectedApp(app);
    setIsModalOpen(true);
  };
    ...
    <div className="p-5">
          ...
          <tbody>
            {filtered.map((app, index) => (
              <tr key={app._id} className="border">
                ...
                {/* STATUS UPDATE (table se direct bhi) */}
                <td className="p-3 border">
                  <select
                    value={app.status || "Pending"}
                    className="border p-1 rounded"
                    onChange={async (e) => {
                      try {
                        ...
                        await api.patch(`/api/v1/resume/${app._id}/status`, {
                          status: newStatus,
                        });
                ...
                <td className="p-3 border flex gap-2">
                  <button
                    onClick={() => handleSendEmailClick(app)} // ✅ pura app object
                    className="bg-green-500 text-white px-3 py-1 rounded"
                  >
                    Send Email
                  </button>
      ...
      {isModalOpen && (
        <Sendemailfrom
          app={selectedApp}
          onClose={() => setIsModalOpen(false)}
          onUpdated={fetchApplications} // ✅ status change hone ke baad list refresh
        />
      )}
    </div>
  );
