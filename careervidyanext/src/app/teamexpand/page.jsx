...
function MentorCard({ mentor, onBook, onDetail }) {
    ...
    const skills =
        mentor.skills || mentor.expertises || mentor.expertise || [];

    const DEFAULT_PHONE = sanitizePhone(
        process.env.NEXT_PUBLIC_DEFAULT_COUNSELLOR_PHONE || "9319998717"
    );
    const mobileNumber =
        sanitizePhone(mentor.mobileNumber) || DEFAULT_PHONE;
                ...
                <div className="flex w-full gap-2">
                    {mobileNumber && (
                        <a
                            ...
                            className="flex-1 flex items-center justify-center p-2 rounded-lg bg-gray-100 hover:bg-gray-200 transition"
                    ...
                    {whatsappNumber && (
                        <a
                            ...
                            className="flex-1 flex items-center justify-center p-2 rounded-lg bg-[#25D366] text-white hover:bg-[#1EBE5A] transition"
                            onClick={(e) => e.stopPropagation()}
...
function FilterSelect({ id, label, value, onChange, options, allLabel }) {
    return (
        <div className="space-y-2">
            <label
                htmlFor={id}
                className="text-xs font-bold text-gray-400 uppercase block"
            >
                {label}
            </label>
            <select
...
export default function TeamExpandContent() {
    ...
    return (
        <div className="flex flex-col min-h-screen">
            ...
            <section
                className="relative bg-gradient-to-r from-[#0f2027] via-[#203a43] to-[#2c5364] h-80 flex items-center justify-center text-center overflow-hidden"
                style={{
                    backgroundImage:
                        "url('/images/counselor-banner-placeholder.jpg')",
                    backgroundSize: "cover",
                    backgroundPosition: "center",
                }}
            >
                <div
                    className="absolute inset-0 bg-gradient-to-r from-[#0f2027] via-[#203a43] to-[#2c5364] backdrop-blur-sm"
                    aria-hidden="true"
                />
                <div className="relative z-10 p-4 max-w-4xl text-white">
                    ...
                    <p className="mt-3 text-lg opacity-90">
                        Find the right mentor to guide your future and book a
                        personalised session today
                    </p>
                </div>
            </section>
                            ...
                            {loading ? (
                                ...
                                <>
                                    <p className="text-sm text-gray-500">
                                        ...
                                        of {filtered.length} counsellors
                                    </p>
                                    {paged.map((m) => (
                                        <MentorCard
                                            key={m._id || m.id}
                                ...
                                <div className="text-center py-20 bg-white rounded-2xl border text-gray-500">
                                    No counsellors found. Try adjusting your
                                    filters.
                                </div>
                            )}
                        </main>
