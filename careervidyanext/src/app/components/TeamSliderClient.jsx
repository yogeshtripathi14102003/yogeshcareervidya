...
export default function TeamSliderClient({ team = [] }) {
  // ✅ FIX: no more slicing the team array. Earlier `SHOW_LIMIT = 6` meant
  // only 6 members were ever rendered in the DOM, while the JSON-LD block
  // below still claimed `numberOfItems: team.length` (the full count).
  // That mismatch between structured data and visible content is exactly
  // the kind of thing that hurts SEO / can get structured data flagged.
  // Now ALL members render — structured data and visible content match.
  const displayedTeam = team;

  // This flag is now purely a LAYOUT decision (slider vs static grid),
  // not a limit on how many members are shown. Everyone still gets
  // rendered either way — this just decides how.
  const useSlider = team.length > 4;
    ...
    <>
      ...
      >
        <div className="max-w-[95%] xl:max-w-[1350px] mx-auto px-4 md:px-8">
          ...
          {/* Dynamic Slider vs Grid Render — layout only, all members always render */}
          {useSlider ? (
            <div className="flex items-center gap-2 md:gap-4">
              <button
                className="swiper-button-prev-team hidden md:flex flex-shrink-0 bg-white/10 backdrop-blur-md p-3 rounded-full shadow-md text-white hover:bg-white hover:text-[#001a41] transition-all z-10 border border-white/20"
