...
//     if (!emailOrPhone || !purpose) {
//       return res.status(400).json({ msg: "Email/Phone and purpose are required." });
//     }
...
//     if (!record) return res.status(400).json({ msg: "No OTP record found. Please resend." });
//     if (record.verified) return res.status(400).json({ msg: "OTP already used." });
//     if (record.expiresAt < new Date()) return res.status(400).json({ msg: "OTP expired." });

//     if (record.attempts >= 5)
//       return res.status(400).json({ msg: "Too many invalid attempts. Request new OTP." });
