...
// ================= UPLOAD DOCUMENTS (FIXED FOR PARALLEL/CHUNK BULK) =================
export const uploadDocuments = async (req, res) => {
  try {
    const admission = await LeadAdmission.findById(req.params.id);

    // Agar admission nahi milta toh clean uploaded file cache immediately
    if (!admission) {
    ...
    // Parallel requests check (Frontend handle karega req.file ya req.files)
    ...
    // Ek file object create karein standard schemas ke according
    const newDoc = {
      fileName: activeFile.originalname,
      fileType: activeFile.mimetype,
      filePath: activeFile.path,
      fileUrl: `${req.protocol}://${req.get("host")}/uploads/documents/${activeFile.filename}`,
      status: "pending",
    };

    // ✅ $push operator atomic locks use karta hai MongoDB me, multiple triggers overlap nahi karenge
    const updatedAdmission = await LeadAdmission.findByIdAndUpdate(
      req.params.id,
      { $push: { documents: newDoc } },
      { new: true }
    );

    // Latest pushed subdocument extraction logic for correct _id distribution
    const savedDoc = updatedAdmission.documents[updatedAdmission.documents.length - 1];

    // Notification documentation creation
    await Notification.create({
      admissionId: updatedAdmission._id,
      documentId: savedDoc._id?.toString(),
      studentName: updatedAdmission.studentName,
      counselorName: updatedAdmission.counselorName,
      fileName: savedDoc.fileName,
      type: "doc_uploaded",
      message: `📄 New document uploaded by ${updatedAdmission.counselorName} for student "${updatedAdmission.studentName}". Please verify: ${savedDoc.fileName}`,
    });

    return res.status(200).json({
      success: true,
      message: "✅ Document uploaded and verification log entry completed.",
      uploadedDocuments: [savedDoc],
      allDocuments: updatedAdmission.documents,
    });
  } catch (error) {
    // Edge case structural catch cleanly
    if (req.file && fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path);
    return res.status(500).json({
      success: false,
      message: "❌ Upload failed: " + error.message,
