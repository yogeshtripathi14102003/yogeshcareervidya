...
// // const defaultJobOpportunity = { heading: "", description: "", jobPost: "", salary: "" };
// // const defaultRecruiter = { companyName: "", packageOffered: "" };
// // const defaultCareerBenefit = { title: "", description: "" };
// // const defaultFaq = { question: "", answer: "" };

// // // ✅ Reusable rich-text field with live preview
// // function RichTextField({ label, value, onChange, placeholder, minHeight = "120px" }) {
...
// //             className="text-xs font-semibold px-3 py-1 rounded-full bg-blue-100 text-blue-700 hover:bg-blue-200"
// //           >
...
// //       )}
// //       {!label && (
// //         <div className="flex justify-end bg-gray-50 px-3 py-1.5 border-b border-gray-200">
// //           <button
// //             type="button"
// //             onClick={() => setShowPreview((p) => !p)}
// //             className="text-xs font-semibold px-3 py-1 rounded-full bg-blue-100 text-blue-700 hover:bg-blue-200"
...
// //         />
// //       ) : (
// //         <ReactQuill theme="snow" value={value} onChange={onChange} placeholder={placeholder} className="bg-white" />
// //       )}
// //     </div>
// //   );
// // }
...
// //   const [jobOpportunities, setJobOpportunities] = useState([defaultJobOpportunity]);
// //   const [topRecruiters, setTopRecruiters] = useState([defaultRecruiter]);
// //   const [careerBenefits, setCareerBenefits] = useState([defaultCareerBenefit]);
...
// //         setDuration(data.duration || "");
// //         setTag(data.tag || "");
// //         setSpecializations(data.specializations?.length > 0 ? data.specializations : [""]);
// //         setCourseLogo_old(data.courseLogo?.url || "");

// //         const mapImageFields = (arr, defaultObj) =>
// //           (arr || []).map((item) => ({
// //             ...defaultObj,
// //             ...item,
// //             image_old: item.image?.url || null,
...
// //         setTopUniversities(data.topUniversities?.length > 0 ? data.topUniversities : [defaultTopUniversity]);
// //         setKeyHighlights(data.keyHighlights?.length > 0 ? data.keyHighlights : [defaultKeyHighlight]);
// //         setSyllabus((data.syllabus || []).map((s) => ({ ...s, subjects: s.subjects?.length > 0 ? s.subjects : [""] })) || [defaultSyllabus]);
// //         setSyllabusPdf_old(data.syllabusPdf?.url || "");
// //         setOfferedCourses((data.offeredCourses || []).map((oc) => ({ ...oc, points: oc.points?.length > 0 ? oc.points : [""] })) || [defaultOfferedCourse]);
// //         setOnlineEligibility(data.onlineEligibility?.length > 0 ? data.onlineEligibility : [defaultEligibility]);
// //         setWorthItDescription(data.onlineCourseWorthIt?.description || "");
// //         setWorthItTopics(data.onlineCourseWorthIt?.topics?.length > 0 ? data.onlineCourseWorthIt.topics : [defaultWorthItTopic]);
// //         setWorthItImage_old(data.onlineCourseWorthIt?.image?.url || "");
// //         setFeeSidebar((data.feeStructureSidebar || []).map((item) => ({ ...item, points: item.points?.length > 0 ? item.points : [""] })) || [defaultFeeSidebar]);
// //         setDetailedFees((data.detailedFees || []).map((item) => ({ ...item, table: item.table?.length > 0 ? item.table : [defaultDetailedFeeTable] })) || [defaultDetailedFee]);
// //         setJobOpportunities(data.jobOpportunities?.length > 0 ? data.jobOpportunities : [defaultJobOpportunity]);
// //         setTopRecruiters(data.topRecruiters?.length > 0 ? data.topRecruiters : [defaultRecruiter]);
// //         setCareerBenefits(data.Careervidyabenifit?.length > 0 ? data.Careervidyabenifit : [defaultCareerBenefit]);
// //         setFaqs(data.faqs?.length > 0 ? data.faqs : [defaultFaq]);
...
// //   const removeSubject = (semesterIndex, subjectIndex) => {
// //     setSyllabus((prev) =>
// //       prev.map((s, i) => (i === semesterIndex ? { ...s, subjects: s.subjects.filter((_, j) => j !== subjectIndex) } : s))
// //     );
// //   };
...
// //       prev.map((oc, i) => (i === blockIndex ? { ...oc, points: oc.points.map((p, j) => (j === pointIndex ? value : p)) } : oc))
// //     );
// //   };
// //   const addOfferedCoursePoint = (index) => {
// //     setOfferedCourses((prev) => prev.map((oc, i) => (i === index ? { ...oc, points: [...oc.points, ""] } : oc)));
...
// //   const removeOfferedCoursePoint = (blockIndex, pointIndex) => {
// //     setOfferedCourses((prev) =>
// //       prev.map((oc, i) => (i === blockIndex ? { ...oc, points: oc.points.filter((_, j) => j !== pointIndex) } : oc))
// //     );
// //   };
...
// //       prev.map((section, si) =>
// //         si === sectionIndex
// //           ? { ...section, table: section.table.map((row, ri) => (ri === rowIndex ? { ...row, [field]: value } : row)) }
// //           : section
// //       )
// //     );
// //   };
// //   const addDetailedFeesTable = (sectionIndex) => {
// //     setDetailedFees((prev) =>
// //       prev.map((section, si) => (si === sectionIndex ? { ...section, table: [...section.table, defaultDetailedFeeTable] } : section))
// //     );
// //   };
// //   const removeDetailedFeesTable = (sectionIndex, rowIndex) => {
// //     setDetailedFees((prev) =>
...
// //   const handleCareerBenefitChange = (index, field, value) => {
// //     setCareerBenefits((prev) => prev.map((item, i) => (i === index ? { ...item, [field]: value } : item)));
// //   };
...
// //       formData.append("overview", prepareArrayForSubmission(overview, "image", "overviewImages", formData));
...
// //       formData.append("Careervidyabenifit", JSON.stringify(filterEmptyObjects(careerBenefits)));
// //       formData.append("faqs", JSON.stringify(filterEmptyObjects(faqs)));
...
// //       console.error("Submission Error:", error);
// //       setMessage(
// //         "❌ Error updating course: " + (error.response?.data?.error || error.response?.data?.message || error.message)
// //       );
// //     } finally {
// //       setLoading(false);
// //     }
...
// //           <h4 className="font-semibold text-lg mb-3 text-green-800 border-b pb-2">Course Overview Blocks</h4>
// //           {overview.map((item, index) => (
// //             <div key={`o-${index}`} className="p-4 mb-4 border border-green-200 rounded-lg bg-white shadow-inner space-y-4">
// //               <div className="grid md:grid-cols-2 gap-4">
// //                 <input type="text" value={item.heading} onChange={(e) => handleObjectArrayChange(setOverview)(index, "heading", e.target.value)} className="border rounded-lg p-2" placeholder="Heading *" required />
// //                 <input type="url" value={item.videoLink} onChange={(e) => handleObjectArrayChange(setOverview)(index, "videoLink", e.target.value)} className="border rounded-lg p-2" placeholder="YouTube Video Link" />
// //               </div>

// //               <RichTextField
// //                 label="Description"
// //                 value={item.description}
// //                 onChange={(val) => handleObjectArrayChange(setOverview)(index, "description", val)}
...
// //           <h4 className="font-semibold text-lg mt-8 mb-3 text-green-800 border-b pb-2">Why Choose Us Points</h4>
// //           {whyChooseUs.map((item, index) => (
// //             <div key={`wc-${index}`} className="p-4 mb-4 border border-green-200 rounded-lg bg-white shadow-inner space-y-4">
// //               <RichTextField
// //                 label="Description"
// //                 value={item.description}
// //                 onChange={(val) => handleObjectArrayChange(setWhyChooseUs)(index, "description", val)}
...
// //                   {item.image_old && !item.image && (<p className="text-xs text-gray-500 mt-1">Current Image: <span className="text-green-700">Present</span></p>)}
// //                 </div>
// //                 {whyChooseUs.length > 1 && (<button type="button" onClick={() => handleObjectArrayRemove(setWhyChooseUs)(index)} className="bg-red-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-red-600">Remove</button>)}
// //               </div>
// //             </div>
// //           ))}
// //           <button type="button" onClick={handleObjectArrayAdd(setWhyChooseUs, defaultWhyChooseUs)} className="bg-green-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-green-700 transition mt-2 shadow-md">+ Add Why Choose Point</button>
...
// //           <h4 className="font-semibold text-lg mb-3 text-yellow-800 border-b pb-2">Key Highlights</h4>
// //           {keyHighlights.map((item, index) => (
// //             <div key={`kh-${index}`} className="p-4 mb-4 border border-yellow-200 rounded-lg bg-white shadow-inner space-y-4">
// //               <div className="grid md:grid-cols-2 gap-4">
// //                 <input type="text" value={item.heading} onChange={(e) => handleObjectArrayChange(setKeyHighlights)(index, "heading", e.target.value)} className="border rounded-lg p-2" placeholder="Main Heading *" required />
// //                 <input type="text" value={item.subHeading} onChange={(e) => handleObjectArrayChange(setKeyHighlights)(index, "subHeading", e.target.value)} className="border rounded-lg p-2" placeholder="Sub-Heading" />
// //               </div>
// //               <RichTextField
// //                 label="Description"
// //                 value={item.description}
// //                 onChange={(val) => handleObjectArrayChange(setKeyHighlights)(index, "description", val)}
...
// //           <h4 className="font-semibold text-lg mt-8 mb-3 text-yellow-800 border-b pb-2">Top Universities Offering This Course</h4>
// //           {topUniversities.map((item, index) => (
// //             <div key={`tu-${index}`} className="p-4 mb-4 border border-yellow-200 rounded-lg bg-white shadow-inner space-y-4">
// //               <input type="text" value={item.name} onChange={(e) => handleObjectArrayChange(setTopUniversities)(index, "name", e.target.value)} className="border rounded-lg p-2 w-full" placeholder="University Name *" required />
// //               <RichTextField
// //                 label="Short Description"
// //                 value={item.description}
// //                 onChange={(val) => handleObjectArrayChange(setTopUniversities)(index, "description", val)}
...
// //           <h4 className="font-semibold text-lg mb-3 text-purple-800 border-b pb-2">Online Eligibility Requirements</h4>
// //           {onlineEligibility.map((item, index) => (
// //             <div key={`e-${index}`} className="p-4 mb-4 border border-purple-200 rounded-lg bg-white shadow-inner space-y-4">
// //               <div className="grid md:grid-cols-2 gap-4">
// //                 <input type="text" value={item.heading} onChange={(e) => handleObjectArrayChange(setOnlineEligibility)(index, "heading", e.target.value)} className="border rounded-lg p-2" placeholder="Heading *" required />
// //                 <input type="text" value={item.subHeading} onChange={(e) => handleObjectArrayChange(setOnlineEligibility)(index, "subHeading", e.target.value)} className="border rounded-lg p-2" placeholder="Sub Heading (e.g., Duration)" />
// //               </div>
// //               <RichTextField
// //                 label="Description / Paragraph Text"
// //                 value={item.description}
// //                 onChange={(val) => handleObjectArrayChange(setOnlineEligibility)(index, "description", val)}
// //                 placeholder="Eligibility description…"
// //               />
// //               <RichTextField
// //                 label="Specific Requirement Text"
// //                 value={item.subDescription}
// //                 onChange={(val) => handleObjectArrayChange(setOnlineEligibility)(index, "subDescription", val)}
// //                 placeholder="Specific requirement details…"
// //                 minHeight="90px"
// //               />
// //               <div className="flex justify-end">
// //                 {onlineEligibility.length > 1 && (<button type="button" onClick={() => handleObjectArrayRemove(setOnlineEligibility)(index)} className="w-full md:w-auto bg-red-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-red-600">Remove</button>)}
// //               </div>
// //             </div>
// //           ))}
// //           <button type="button" onClick={handleObjectArrayAdd(setOnlineEligibility, defaultEligibility)} className="bg-purple-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-purple-700 transition mt-2 shadow-md">+ Add Eligibility Block</button>
...
// //           <div className="mb-6">
// //             <RichTextField
// //               label="Main Introductory Description"
// //               value={worthItDescription}
// //               onChange={(val) => setWorthItDescription(val)}
// //               placeholder="Main introductory description…"
// //             />
// //           </div>

// //           <label className="block font-medium mt-3 mb-1 text-sm">Worth It Topics</label>
// //           {worthItTopics.map((topic, index) => (
// //             <div key={`wt-${index}`} className="mb-4 p-4 border border-purple-200 rounded-lg bg-white shadow-inner space-y-4">
// //               <input type="text" value={topic.subHeading} onChange={(e) => handleObjectArrayChange(setWorthItTopics)(index, "subHeading", e.target.value)} className="border rounded-lg p-2 w-full" placeholder="Topic Heading (e.g., Flexibility)" />
// //               <RichTextField
// //                 label="Topic Description"
// //                 value={topic.description}
// //                 onChange={(val) => handleObjectArrayChange(setWorthItTopics)(index, "description", val)}
// //                 placeholder="Topic description…"
// //                 minHeight="90px"
// //               />
// //               <div className="flex justify-end">
// //                 {worthItTopics.length > 1 && (<button type="button" onClick={() => handleObjectArrayRemove(setWorthItTopics)(index)} className="w-full md:w-auto bg-red-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-red-600">Remove</button>)}
// //               </div>
// //             </div>
// //           ))}
// //           <button type="button" onClick={handleObjectArrayAdd(setWorthItTopics, defaultWorthItTopic)} className="bg-purple-800 text-white px-4 py-2 rounded-lg text-sm hover:bg-purple-900 transition mt-2 shadow-md">+ Add Worth It Topic</button>
...
// //           <h4 className="font-semibold text-lg mt-8 mb-3 text-red-800 border-b pb-2">Detailed Fees Tables</h4>
// //           {detailedFees.map((section, sectionIndex) => (
// //             <div key={`df-${sectionIndex}`} className="p-4 mb-4 border border-red-200 rounded-lg bg-white shadow-inner space-y-4">
// //               <h5 className="font-semibold text-md mb-2 flex justify-between items-center">
// //                 <span className="text-red-700">Section Heading:</span>
// //                 <input type="text" value={section.heading} onChange={(e) => handleObjectArrayChange(setDetailedFees)(sectionIndex, "heading", e.target.value)} className="border rounded-lg p-1 w-1/2" placeholder="e.g. Total Fees Breakdown" required />
// //                 {detailedFees.length > 1 && (<button type="button" onClick={() => handleObjectArrayRemove(setDetailedFees)(sectionIndex)} className="text-red-600 text-sm hover:text-red-800">Remove Section</button>)}
...
// //                 onChange={(val) => handleObjectArrayChange(setDetailedFees)(sectionIndex, "description", val)}
...
// //           <h4 className="font-semibold text-lg mb-3 text-indigo-800 border-b pb-2">Job Opportunities</h4>
// //           {jobOpportunities.map((job, index) => (
// //             <div key={`jo-${index}`} className="mb-4 p-4 border border-indigo-200 rounded-lg bg-white shadow-inner space-y-4">
// //               <div className="grid md:grid-cols-2 gap-4">
// //                 <input type="text" value={job.jobPost} onChange={(e) => handleObjectArrayChange(setJobOpportunities)(index, "jobPost", e.target.value)} className="border rounded-lg p-2" placeholder="Job Role" />
// //                 <input type="text" value={job.salary} onChange={(e) => handleObjectArrayChange(setJobOpportunities)(index, "salary", e.target.value)} className="border rounded-lg p-2" placeholder="Salary Range" />
// //               </div>
// //               <div className="flex justify-end">
// //                 {jobOpportunities.length > 1 && (<button type="button" onClick={() => handleObjectArrayRemove(setJobOpportunities)(index)} className="bg-red-500 text-white px-3 py-2 rounded-lg hover:bg-red-600 text-sm">Remove</button>)}
// //               </div>
...
// //         {/* 8. CAREER VIDYA BENEFITS */}
// //         <div className="p-6 border border-cyan-400 rounded-xl bg-cyan-50 shadow-sm">
// //           <h3 className="font-bold text-xl mb-4 text-[#006064]">8. Career Vidya Benefits</h3>
// //           <p className="text-sm text-gray-600 mb-4">These show up in the EMI/benefits section on the course page.</p>

// //           {careerBenefits.map((item, index) => (
// //             <div key={`cb-${index}`} className="p-4 mb-4 border border-cyan-200 rounded-lg bg-white shadow-inner space-y-3">
// //               <div className="flex justify-between items-center gap-2">
// //                 <input
// //                   type="text"
// //                   value={item.title}
// //                   onChange={(e) => handleCareerBenefitChange(index, "title", e.target.value)}
// //                   className="border rounded-lg p-2 flex-1"
// //                   placeholder="Benefit Title (e.g., No Cost EMI)"
// //                 />
// //                 {careerBenefits.length > 1 && (
// //                   <button type="button" onClick={() => handleObjectArrayRemove(setCareerBenefits)(index)} className="bg-red-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-red-600 whitespace-nowrap">
// //                     Remove
// //                   </button>
// //                 )}
// //               </div>
...
// //                 label="Description"
// //                 value={item.description}
// //                 onChange={(val) => handleCareerBenefitChange(index, "description", val)}
// //                 placeholder="Describe this benefit…"
// //               />
// //             </div>
// //           ))}
// //           <button type="button" onClick={handleObjectArrayAdd(setCareerBenefits, defaultCareerBenefit)} className="bg-cyan-700 text-white px-4 py-2 rounded-lg text-sm hover:bg-cyan-800 transition mt-2 shadow-md">
// //             + Add Career Benefit
// //           </button>
// //         </div>
...
// //           <h3 className="font-bold text-xl mb-4 text-[#9A3412]">9. Frequently Asked Questions</h3>

// //           {faqs.map((item, index) => (
// //             <div key={`faq-${index}`} className="p-4 mb-4 border border-orange-200 rounded-lg bg-white shadow-inner space-y-3">
// //               <div className="flex justify-between items-center gap-2">
// //                 <input
// //                   type="text"
// //                   value={item.question}
...
// //                 />
// //                 {faqs.length > 1 && (
// //                   <button type="button" onClick={() => handleObjectArrayRemove(setFaqs)(index)} className="bg-red-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-red-600 whitespace-nowrap">
// //                     Remove
// //                   </button>
// //                 )}
// //               </div>
...
// //             </div>
// //           ))}
// //           <button type="button" onClick={handleObjectArrayAdd(setFaqs, defaultFaq)} className="bg-orange-700 text-white px-4 py-2 rounded-lg text-sm hover:bg-orange-800 transition mt-2 shadow-md">
// //             + Add FAQ
// //           </button>
// //         </div>
...
import { useState, useEffect } from "react";
import api from "@/utlis/api.js";
// import UniversitySelector from "@/app/admin/getonlinecourese/UniversitySelector.jsx"
import UniversitiesFetchComponent from "@/app/admin/getonlinecourese/UniversitiesFetchComponent.jsx";

import RichTextField from "@/app/admin/components/RichTextField.jsx"; // ✅ Shared component
...
const defaultEligibility = { heading: "", description: "", subHeading: "", subDescription: "" };
const defaultFeeSidebar = { heading: "", points: [""] };
const defaultDetailedFeeRow = { universityName: "", courseFees: "", detailedFeeStructure: "" };
const defaultDetailedFee = { heading: "", description: "", table: [defaultDetailedFeeRow] };
const defaultWorthItTopic = { subHeading: "", description: "" };
const defaultJobOpportunity = { heading: "", description: "", jobPost: "", salary: "" };
const defaultRecruiter = { companyName: "", packageOffered: "" };
const defaultCareerBenefit = { title: "", description: "" };
...
// NEW defaults
const defaultAdmissionStep = { step: 1, title: "", description: "" };
const defaultSpecDetail = {
  name: "",
  description: "",
  careerRoles: [{ role: "", salary: "" }],
};
const defaultScholarship = { title: "", discount: "", description: "" };
...
const defaultPlacementSupport = {
  description: "",
  stats: { placementRate: "", avgPackage: "", highestPackage: "" },
};
...
export default function Editcourse({ courseId, onClose, onUpdated }) {
  ...
  // NEW FIELDS
  const [admissionProcess, setAdmissionProcess] = useState([defaultAdmissionStep]);
  const [specializationDetails, setSpecializationDetails] = useState([defaultSpecDetail]);
  const [emiOptions, setEmiOptions] = useState(defaultEmiOptions);
  const [scholarships, setScholarships] = useState([defaultScholarship]);
  const [courseTestimonials, setCourseTestimonials] = useState([defaultTestimonial]);
  const [placementSupport, setPlacementSupport] = useState(defaultPlacementSupport);
  ...
  useEffect(() => {
    ...
    const fetchCourseData = async () => {
      try {
        ...
        // Overview
        const mappedOverview = mapImageFields(data.overview, defaultOverview);
        setOverview(mappedOverview.length > 0 ? mappedOverview : [defaultOverview]);

        // Why
        const mappedWhy = mapImageFields(data.whyChooseUs, defaultWhyChooseUs);
        setWhyChooseUs(mappedWhy.length > 0 ? mappedWhy : [defaultWhyChooseUs]);

        // Simple arrays
        setGoodThings(data.goodThings?.length > 0 ? data.goodThings : [""]);
        setKeyHighlights(
          data.keyHighlights?.length > 0 ? data.keyHighlights : [defaultKeyHighlight]
        );

        // Syllabus
        const mappedSyllabus = (data.syllabus || []).map((s) => ({
          ...s,
          subjects: s.subjects?.length > 0 ? s.subjects : [""],
        }));
        setSyllabus(mappedSyllabus.length > 0 ? mappedSyllabus : [defaultSyllabus]);
        setSyllabusPdf_old(data.syllabusPdf?.url || "");

        // Offered
        const mappedOffered = (data.offeredCourses || []).map((oc) => ({
          ...oc,
          points: oc.points?.length > 0 ? oc.points : [""],
        }));
        setOfferedCourses(mappedOffered.length > 0 ? mappedOffered : [defaultOfferedCourse]);

        // Eligibility
        setOnlineEligibility(
          data.onlineEligibility?.length > 0 ? data.onlineEligibility : [defaultEligibility]
        );

        // Worth It
        setWorthItDescription(data.onlineCourseWorthIt?.description || "");
        setWorthItTopics(
          data.onlineCourseWorthIt?.topics?.length > 0
            ? data.onlineCourseWorthIt.topics
            : [defaultWorthItTopic]
        );
        setWorthItImage_old(data.onlineCourseWorthIt?.image?.url || "");

        // Fee Sidebar
        const mappedFeeSide = (data.feeStructureSidebar || []).map((item) => ({
          ...item,
          points: item.points?.length > 0 ? item.points : [""],
        }));
        setFeeSidebar(mappedFeeSide.length > 0 ? mappedFeeSide : [defaultFeeSidebar]);

        // Detailed Fees
        const mappedFees = (data.detailedFees || []).map((item) => ({
          ...item,
          table: item.table?.length > 0 ? item.table : [defaultDetailedFeeRow],
        }));
        setDetailedFees(mappedFees.length > 0 ? mappedFees : [defaultDetailedFee]);

        // Jobs
        setJobOpportunities(
          data.jobOpportunities?.length > 0 ? data.jobOpportunities : [defaultJobOpportunity]
        );
        setTopRecruiters(
          data.topRecruiters?.length > 0 ? data.topRecruiters : [defaultRecruiter]
        );

        // Career
        setCareerBenefits(
          data.Careervidyabenifit?.length > 0
            ? data.Careervidyabenifit
            : [defaultCareerBenefit]
        );
        ...
        // NEW: Admission
        setAdmissionProcess(
          data.admissionProcess?.length > 0 ? data.admissionProcess : [defaultAdmissionStep]
        );

        // NEW: Spec Details
        const mappedSpecDetails = (data.specializationDetails || []).map((s) => ({
          ...s,
          careerRoles:
            s.careerRoles?.length > 0 ? s.careerRoles : [{ role: "", salary: "" }],
        }));
        setSpecializationDetails(
          mappedSpecDetails.length > 0 ? mappedSpecDetails : [defaultSpecDetail]
        );

        // NEW: EMI
        setEmiOptions({
          enabled: data.emiOptions?.enabled || false,
          minMonthly: data.emiOptions?.minMonthly || "",
          maxMonthly: data.emiOptions?.maxMonthly || "",
          tenureMonths: Array.isArray(data.emiOptions?.tenureMonths)
            ? data.emiOptions.tenureMonths.join(",")
            : "",
        });

        // NEW: Scholarships
        setScholarships(
          data.scholarships?.length > 0 ? data.scholarships : [defaultScholarship]
        );

        // NEW: Testimonials
        const mappedTestimonials = (data.courseTestimonials || []).map((t) => ({
          ...t,
          image_old: t.image?.url || null,
          image: null,
        }));
        setCourseTestimonials(
          mappedTestimonials.length > 0 ? mappedTestimonials : [defaultTestimonial]
        );

        // NEW: Placement
        setPlacementSupport({
          description: data.placementSupport?.description || "",
          stats: {
            placementRate: data.placementSupport?.stats?.placementRate || "",
            avgPackage: data.placementSupport?.stats?.avgPackage || "",
            highestPackage: data.placementSupport?.stats?.highestPackage || "",
          },
        });
  ...
  const handleObjectArrayFileChange = (setter) => (index, field, file) =>
    setter((prev) =>
      prev.map((item, i) =>
        i === index ? { ...item, [field]: file, [`${field}_old`]: null } : item
      )
    );
    ...
    setSyllabus((prev) =>
      prev.map((s, i) =>
        i === semIndex
          ? {
              ...s,
              subjects: s.subjects.map((sub, j) => (j === subIndex ? value : sub)),
            }
          : s
      )
    );
  ...
  const addSubject = (index) =>
    setSyllabus((prev) =>
      prev.map((s, i) => (i === index ? { ...s, subjects: [...s.subjects, ""] } : s))
    );
  ...
  const removeSubject = (semIndex, subIndex) =>
    setSyllabus((prev) =>
      prev.map((s, i) =>
        i === semIndex
          ? { ...s, subjects: s.subjects.filter((_, j) => j !== subIndex) }
          : s
      )
    );
    ...
    setOfferedCourses((prev) =>
      prev.map((oc, i) =>
        i === blockIndex
          ? {
              ...oc,
              points: oc.points.map((p, j) => (j === pointIndex ? value : p)),
            }
          : oc
      )
    );
  ...
  const addOfferedCoursePoint = (index) =>
    setOfferedCourses((prev) =>
      prev.map((oc, i) => (i === index ? { ...oc, points: [...oc.points, ""] } : oc))
    );
  ...
  const removeOfferedCoursePoint = (blockIndex, pointIndex) =>
    setOfferedCourses((prev) =>
      prev.map((oc, i) =>
        i === blockIndex
          ? { ...oc, points: oc.points.filter((_, j) => j !== pointIndex) }
          : oc
      )
    );
    ...
    setDetailedFees((prev) =>
      prev.map((section, si) =>
        si === sectionIndex
          ? {
              ...section,
              table: section.table.map((row, ri) =>
                ri === rowIndex ? { ...row, [field]: value } : row
              ),
            }
          : section
  ...
  const addDetailedFeesTable = (sectionIndex) =>
    setDetailedFees((prev) =>
      prev.map((section, si) =>
        si === sectionIndex
          ? { ...section, table: [...section.table, defaultDetailedFeeRow] }
          : section
      )
    );
  ...
  const removeDetailedFeesTable = (sectionIndex, rowIndex) =>
    setDetailedFees((prev) =>
      prev.map((section, si) =>
        si === sectionIndex
          ? { ...section, table: section.table.filter((_, ri) => ri !== rowIndex) }
          : section
      )
    );
  ...
  /* -------------------- Spec Details -------------------- */
  const handleSpecDetailChange = (i, field, value) =>
    setSpecializationDetails((prev) =>
      prev.map((item, idx) => (idx === i ? { ...item, [field]: value } : item))
    );

  const handleSpecCareerRoleChange = (specIdx, roleIdx, field, value) =>
    setSpecializationDetails((prev) =>
      prev.map((spec, i) =>
        i === specIdx
          ? {
              ...spec,
              careerRoles: spec.careerRoles.map((role, j) =>
                j === roleIdx ? { ...role, [field]: value } : role
              ),
            }
          : spec
      )
  ...
  const addSpecCareerRole = (specIdx) =>
    setSpecializationDetails((prev) =>
      prev.map((spec, i) =>
        i === specIdx
          ? { ...spec, careerRoles: [...spec.careerRoles, { role: "", salary: "" }] }
          : spec
      )
    );

  const removeSpecCareerRole = (specIdx, roleIdx) =>
    setSpecializationDetails((prev) =>
      prev.map((spec, i) =>
        i === specIdx
          ? {
              ...spec,
              careerRoles: spec.careerRoles.filter((_, j) => j !== roleIdx),
            }
          : spec
      )
    );
  ...
  const handleTestimonialImage = (i, file) =>
    setCourseTestimonials((prev) =>
      prev.map((item, idx) =>
        idx === i ? { ...item, image: file, image_old: null } : item
      )
    );

  /* -------------------- Placement -------------------- */
  const handlePlacementChange = (field, value) =>
    setPlacementSupport((prev) => ({ ...prev, [field]: value }));

  const handlePlacementStatChange = (field, value) =>
    setPlacementSupport((prev) => ({
      ...prev,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append(
        "specializations",
        JSON.stringify(filterEmptyObjects(specializations))
      );
      ...
      formData.append(
        "keyHighlights",
        JSON.stringify(filterEmptyObjects(keyHighlights))
      );
      ...
      // Syllabus
      const cleanedSyllabus = syllabus.map((s) => ({
        ...s,
        subjects: filterEmptyObjects(s.subjects),
      }));
      formData.append("syllabus", JSON.stringify(filterEmptyObjects(cleanedSyllabus)));
      if (syllabusPdf instanceof File) formData.append("syllabusPdf", syllabusPdf);

      // Offered
      const cleanedOffered = offeredCourses.map((oc) => ({
        ...oc,
        points: filterEmptyObjects(oc.points),
      }));
      formData.append("offeredCourses", JSON.stringify(filterEmptyObjects(cleanedOffered)));

      // Eligibility
      formData.append(
        "onlineEligibility",
        JSON.stringify(filterEmptyObjects(onlineEligibility))
      );
      ...
      formData.append(
        "admissionProcess",
        JSON.stringify(
          admissionProcess.filter((s) => s.title).map((s, i) => ({ ...s, step: i + 1 }))
        )
      );
      ...
      if (worthItImage instanceof File)
        formData.append("onlineCourseWorthItImage", worthItImage);

      // Fee Sidebar
      const cleanedFeeSidebar = feeSidebar.map((item) => ({
        ...item,
        points: filterEmptyObjects(item.points),
      }));
      formData.append(
        "feeStructureSidebar",
        JSON.stringify(filterEmptyObjects(cleanedFeeSidebar))
      );

      // Detailed Fees
      const cleanedDetailedFees = detailedFees.map((section) => ({
        ...section,
        table: filterEmptyObjects(section.table),
      }));
      formData.append(
        "detailedFees",
        JSON.stringify(filterEmptyObjects(cleanedDetailedFees))
      );
      ...
      formData.append(
        "emiOptions",
        JSON.stringify({
          enabled: emiOptions.enabled,
          minMonthly: emiOptions.minMonthly,
          maxMonthly: emiOptions.maxMonthly,
          tenureMonths: emiOptions.tenureMonths
            ? emiOptions.tenureMonths
                .split(",")
                .map((n) => Number(n.trim()))
                .filter(Boolean)
            : [],
      ...
      formData.append(
        "scholarships",
        JSON.stringify(scholarships.filter((s) => s.title))
      );

      // Jobs
      formData.append(
        "jobOpportunities",
        JSON.stringify(filterEmptyObjects(jobOpportunities))
      );
      formData.append(
        "topRecruiters",
        JSON.stringify(filterEmptyObjects(topRecruiters))
      );
      ...
      courseTestimonials.forEach((t) => {
        if (t.image instanceof File)
          formData.append("courseTestimonialImages", t.image);
      });

      // Career
      formData.append(
        "Careervidyabenifit",
        JSON.stringify(filterEmptyObjects(careerBenefits))
      );
      ...
      // Submit
      await api.put(`/api/v1/course/${courseId}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
  ...
  if (dataLoadingError) {
    return (
      <div className="max-w-4xl w-full p-8 text-center text-xl font-semibold bg-white rounded-xl">
        ...
        <button
          onClick={onClose}
          className="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded-lg text-sm block mx-auto mt-4"
        >
          Close
        </button>
      </div>
  ...
  return (
    <div className="max-w-6xl w-full mx-auto p-8 bg-white shadow-2xl rounded-xl">
      <div className="flex justify-between items-center mb-6 border-b pb-4">
        <h2 className="text-3xl font-extrabold text-[#002D62]">
          ✍️ Edit Course: {name}
        </h2>
        <button
          onClick={onClose}
      ...
      <form onSubmit={handleSubmit} className="space-y-12">
        {/* ============ 1. CORE ============ */}
        <div className="p-6 border border-blue-400 rounded-xl bg-blue-50 shadow-sm">
          <h3 className="font-bold text-xl mb-4 text-[#002D62]">
            1. Core Details & Metadata
          </h3>
          <div className="grid md:grid-cols-3 gap-4 mb-4">
            <input
          ...
          <h4 className="font-semibold text-lg mt-4 mb-2 text-blue-800">
          ...
          {specializations.map((spec, index) => (
            <div key={index} className="flex gap-2 mb-2 items-center">
              <input
                ...
                value={spec}
                onChange={(e) =>
                  handleStringArrayChange(setSpecializations)(index, e.target.value)
                }
              ...
              {specializations.length > 1 && (
                <button
                  type="button"
                  onClick={() => handleStringArrayRemove(setSpecializations)(index)}
          ...
          <button
            type="button"
            onClick={handleStringArrayAdd(setSpecializations, "")}
            className="bg-blue-600 text-white px-3 py-1 rounded-lg text-sm hover:bg-blue-700 mt-2 shadow-md"
          >
            + Add Specialization
          </button>
          ...
          <div className="mt-6 pt-4 border-t border-blue-200">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setCourseLogo_old(null);
              }}
              className="w-full text-gray-700 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-100 file:text-[#002D62] hover:file:bg-blue-200 cursor-pointer"
            />
            {courseLogo_old && !courseLogo && (
              <p className="text-sm text-gray-500 mt-1">
                Current Logo: <span className="font-semibold text-blue-700">Present</span>
              </p>
            )}
          </div>
        </div>
        ...
        {/* ============ 2. UNIVERSITIES ============ */}
        <div className="p-6 border-2 border-purple-400 rounded-xl bg-purple-50 shadow-sm">
          <h3 className="font-bold text-xl mb-4 text-[#4B0082]">
            2. 🎯 Universities for this Course
          </h3>
          <p className="text-sm text-gray-600 mb-4">
            Select universities from database. Har selected university ke liye
            course-specific details (fees, apply link, etc.) bhare.
          </p>

          <UniversitiesFetchComponent value={universities} onChange={setUniversities} />
        </div>

        {/* ============ 3. SPECIALIZATION DETAILS ============ */}
        <div className="p-6 border border-pink-400 rounded-xl bg-pink-50 shadow-sm">
          <h3 className="font-bold text-xl mb-4 text-[#9D174D]">
            3. 🎓 Specialization Details (Advanced)
          </h3>
          {specializationDetails.map((spec, i) => (
            <div
              key={i}
              className="p-4 mb-4 border border-pink-200 rounded-lg bg-white shadow-inner space-y-3"
            >
              <div className="flex gap-2">
                <input
                  type="text"
                  value={spec.name}
                  onChange={(e) => handleSpecDetailChange(i, "name", e.target.value)}
                  className="border rounded-lg p-2 flex-1"
                  placeholder="Specialization name"
                />
                {specializationDetails.length > 1 && (
                  <button
                    type="button"
                    onClick={() => handleObjectArrayRemove(setSpecializationDetails)(i)}
              ...
              <RichTextField
                ...
                onChange={(val) => handleSpecDetailChange(i, "description", val)}
              ...
              {spec.careerRoles.map((role, j) => (
                <div key={j} className="grid md:grid-cols-3 gap-2 items-center">
                  <input
                    type="text"
                    value={role.role}
                    onChange={(e) =>
                      handleSpecCareerRoleChange(i, j, "role", e.target.value)
                    }
                    className="border rounded-lg p-2"
                    placeholder="Role (Data Analyst)"
                  />
                  <input
                    type="text"
                    value={role.salary}
                    onChange={(e) =>
                      handleSpecCareerRoleChange(i, j, "salary", e.target.value)
                    }
                    className="border rounded-lg p-2"
                    placeholder="Salary (₹3-6 LPA)"
                  />
                  {spec.careerRoles.length > 1 && (
                    <button
                      type="button"
                      onClick={() => removeSpecCareerRole(i, j)}
          ...
          <button
            type="button"
            onClick={handleObjectArrayAdd(setSpecializationDetails, defaultSpecDetail)}
            className="bg-pink-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-pink-700 mt-2 shadow-md"
          >
        ...
        {/* ============ 4. OVERVIEW & WHY ============ */}
        <div className="p-6 border border-green-400 rounded-xl bg-green-50 shadow-sm">
          <h3 className="font-bold text-xl mb-4 text-[#006400]">
            4. Overview & Selling Points
          </h3>

          <h4 className="font-semibold text-lg mb-3 text-green-800 border-b pb-2">
            Course Overview Blocks
          </h4>
          {overview.map((item, index) => (
            <div
              key={`o-${index}`}
              className="p-4 mb-4 border border-green-200 rounded-lg bg-white shadow-inner space-y-4"
            >
              <div className="grid md:grid-cols-2 gap-4">
                <input
                  type="text"
                  value={item.heading}
                  onChange={(e) =>
                    handleObjectArrayChange(setOverview)(index, "heading", e.target.value)
                  }
                  className="border rounded-lg p-2"
                  placeholder="Heading *"
                  required
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleObjectArrayChange(setOverview)(index, "videoLink", e.target.value)
                  }
                  className="border rounded-lg p-2"
              ...
              <RichTextField
                ...
                value={item.description}
                onChange={(val) =>
                  handleObjectArrayChange(setOverview)(index, "description", val)
                }
                placeholder="Course overview description…"
              />
              <div className="grid md:grid-cols-4 gap-4 items-end">
                <div className="col-span-3">
                  ...
                  <input
                    ...
                    accept="image/*"
                    onChange={(e) =>
                      handleObjectArrayFileChange(setOverview)(
                        index,
                        "image",
                        e.target.files[0]
                      )
                    }
                    className="w-full text-gray-700 file:py-1 file:px-2 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-green-100 file:text-green-700 cursor-pointer"
                  />
                  {item.image_old && !item.image && (
                    <p className="text-xs text-gray-500 mt-1">
                      Current Image: <span className="text-green-700">Present</span>
                    </p>
                  )}
                </div>
                <div className="col-span-1">
                  {overview.length > 1 && (
                    <button
                      type="button"
                      onClick={() => handleObjectArrayRemove(setOverview)(index)}
                      className="w-full bg-red-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-red-600"
                    >
                      Remove
                    </button>
                  )}
          ...
          <button
            type="button"
            onClick={handleObjectArrayAdd(setOverview, defaultOverview)}
            className="bg-green-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-green-700 transition mt-2 shadow-md"
          >
            + Add Overview Block
          </button>

          <h4 className="font-semibold text-lg mt-8 mb-3 text-green-800 border-b pb-2">
          ...
          {whyChooseUs.map((item, index) => (
            ...
            >
              <RichTextField
                ...
                value={item.description}
                onChange={(val) =>
                  handleObjectArrayChange(setWhyChooseUs)(index, "description", val)
                }
                placeholder="Why choose this course…"
              />
              <div className="grid md:grid-cols-2 gap-4 items-end">
                <div>
                  ...
                  <input
                    ...
                    accept="image/*"
                    onChange={(e) =>
                      handleObjectArrayFileChange(setWhyChooseUs)(
                        index,
                        "image",
                        e.target.files[0]
                      )
                ...
                {whyChooseUs.length > 1 && (
                  <button
                    type="button"
                    onClick={() => handleObjectArrayRemove(setWhyChooseUs)(index)}
                    className="bg-red-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-red-600"
                  >
                    Remove
                  </button>
          ...
          <button
            type="button"
            onClick={handleObjectArrayAdd(setWhyChooseUs, defaultWhyChooseUs)}
            className="bg-green-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-green-700 transition mt-2 shadow-md"
          >
          ...
          {goodThings.map((item, index) => (
            <div key={`gt-${index}`} className="flex gap-2 mb-2 items-center">
              <input
                ...
                value={item}
                onChange={(e) =>
                  handleStringArrayChange(setGoodThings)(index, e.target.value)
                }
                className="w-full border rounded-lg p-2 border-gray-300"
              ...
              {goodThings.length > 1 && (
                <button
                  type="button"
                  onClick={() => handleStringArrayRemove(setGoodThings)(index)}
                  className="text-red-600 hover:text-red-800"
                >
                  Remove
                </button>
          ...
          <button
            type="button"
            onClick={handleStringArrayAdd(setGoodThings, "")}
            className="bg-green-800 text-white px-3 py-1 rounded-lg text-sm hover:bg-green-900 mt-2 shadow-md"
          >
            + Add Good Thing
          </button>
        </div>
        ...
        {/* ============ 5. HIGHLIGHTS ============ */}
        <div className="p-6 border border-yellow-400 rounded-xl bg-yellow-50 shadow-sm">
          <h3 className="font-bold text-xl mb-4 text-[#CC6600]">
            5. Key Highlights
          </h3>
          ...
          {keyHighlights.map((item, index) => (
            <div
              key={`kh-${index}`}
              className="p-4 mb-4 border border-yellow-200 rounded-lg bg-white shadow-inner space-y-4"
            >
              <div className="grid md:grid-cols-2 gap-4">
                <input
                  type="text"
                  value={item.heading}
                  onChange={(e) =>
                    handleObjectArrayChange(setKeyHighlights)(
                      index,
                      "heading",
                      e.target.value
                    )
              ...
              <RichTextField
                ...
                value={item.description}
                onChange={(val) =>
                  handleObjectArrayChange(setKeyHighlights)(index, "description", val)
                }
                placeholder="Highlight description…"
              />
              <div className="flex justify-end">
                {keyHighlights.length > 1 && (
                  <button
                    type="button"
                    onClick={() => handleObjectArrayRemove(setKeyHighlights)(index)}
                    className="bg-red-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-red-600"
                  >
                    Remove
                  </button>
          ...
          <button
            type="button"
            onClick={handleObjectArrayAdd(setKeyHighlights, defaultKeyHighlight)}
            className="bg-yellow-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-yellow-700 transition mt-2 shadow-md"
          >
            + Add Key Highlight
          </button>
        </div>
        ...
        {/* ============ 6. SYLLABUS & OFFERED ============ */}
        <div className="p-6 border border-teal-400 rounded-xl bg-teal-50 shadow-sm">
          <h3 className="font-bold text-xl mb-4 text-[#008080]">
            6. Syllabus Structure & Content
          </h3>

          <h4 className="font-semibold text-lg mb-3 text-teal-800 border-b pb-2">
            Syllabus by Semester/Module
          </h4>
          {syllabus.map((semester, semIndex) => (
            <div
              key={`s-${semIndex}`}
              className="p-4 mb-4 border border-teal-200 rounded-lg bg-white shadow-inner"
            >
              <h5 className="font-semibold text-md mb-2 flex justify-between items-center">
                <span className="text-teal-700">Semester/Module:</span>
                <input
                  ...
                  onChange={(e) =>
                    handleSyllabusChange(semIndex, "semester", e.target.value)
                  }
                ...
                {syllabus.length > 1 && (
                  <button
                    type="button"
                    onClick={() =>
                      setSyllabus(syllabus.filter((_, i) => i !== semIndex))
                    }
                    className="text-red-600 text-sm hover:text-red-800"
                  >
                    Remove Semester
              ...
              {semester.subjects.map((subject, subIndex) => (
                <div key={`subj-${subIndex}`} className="flex gap-2 mb-2 items-center">
                  <input
                    ...
                    value={subject}
                    onChange={(e) =>
                      handleSubjectChange(semIndex, subIndex, e.target.value)
                    }
              ...
              <button
                ...
                className="bg-teal-600 text-white px-3 py-1 rounded-lg text-xs hover:bg-teal-700 mt-2 shadow-sm"
              >
                + Add Subject
              </button>
            </div>
          ...
          <button
            type="button"
            onClick={() => setSyllabus((prev) => [...prev, defaultSyllabus])}
            className="bg-[#008080] text-white px-4 py-2 rounded-lg text-sm hover:bg-teal-700 transition mt-2 shadow-md"
          >
            + Add New Semester
          </button>
          ...
          <div className="mt-6 pt-4 border-t border-teal-200">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setSyllabusPdf_old(null);
              }}
              className="w-full text-gray-700 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-teal-100 file:text-[#008080] hover:file:bg-teal-200 cursor-pointer"
            />
            {syllabusPdf_old && !syllabusPdf && (
              <p className="text-sm text-gray-500 mt-1">
                Current PDF:{" "}
                <a
                  href={syllabusPdf_old}
                  target="_blank"
                  rel="noreferrer"
          ...
          <h4 className="font-semibold text-lg mt-8 mb-3 text-teal-800 border-b pb-2">
            Offered Courses (List Blocks)
          </h4>
          {offeredCourses.map((oc, blockIndex) => (
            <div
              key={`oc-${blockIndex}`}
            ...
            >
              <h5 className="font-semibold text-md mb-2 flex justify-between items-center">
                ...
                <input
                  ...
                  value={oc.heading}
                  onChange={(e) =>
                    handleOfferedCourseChange(blockIndex, "heading", e.target.value)
                  }
                  className="border rounded-lg p-1 w-1/2"
                  placeholder="e.g. UG Programs"
                  required
                />
                {offeredCourses.length > 1 && (
                  <button
                    type="button"
                    onClick={() => handleObjectArrayRemove(setOfferedCourses)(blockIndex)}
                    className="text-red-600 text-sm hover:text-red-800"
                  >
                    Remove Block
                  </button>
          ...
          <button
            type="button"
            onClick={handleObjectArrayAdd(setOfferedCourses, defaultOfferedCourse)}
            className="bg-teal-800 text-white px-4 py-2 rounded-lg text-sm hover:bg-teal-900 transition mt-2 shadow-md"
          >
            + Add New Offered Courses Block
          </button>
        </div>

        {/* ============ 7. ELIGIBILITY & WORTH IT ============ */}
        <div className="p-6 border border-purple-400 rounded-xl bg-purple-50 shadow-sm">
          <h3 className="font-bold text-xl mb-4 text-[#6A0DAD]">
            7. Eligibility & Course Value
          </h3>

          <h4 className="font-semibold text-lg mb-3 text-purple-800 border-b pb-2">
            Online Eligibility Requirements
          </h4>
          {onlineEligibility.map((item, index) => (
            <div
              key={`e-${index}`}
              className="p-4 mb-4 border border-purple-200 rounded-lg bg-white shadow-inner space-y-4"
            >
              <div className="grid md:grid-cols-2 gap-4">
                <input
                  type="text"
                  value={item.heading}
                  onChange={(e) =>
                    handleObjectArrayChange(setOnlineEligibility)(
                      index,
                      "heading",
                      e.target.value
                    )
                ...
                <input
                  ...
                  className="border rounded-lg p-2"
                  placeholder="Sub Heading (e.g., Duration)"
                />
              </div>
              <RichTextField
                label="Description / Paragraph Text"
              ...
              />
              <div className="flex justify-end">
                {onlineEligibility.length > 1 && (
                  <button
                    type="button"
                    onClick={() => handleObjectArrayRemove(setOnlineEligibility)(index)}
                    className="w-full md:w-auto bg-red-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-red-600"
                  >
                    Remove
                  </button>
                )}
          ...
          <button
            type="button"
            onClick={handleObjectArrayAdd(setOnlineEligibility, defaultEligibility)}
            className="bg-purple-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-purple-700 transition mt-2 shadow-md"
          >
            + Add Eligibility Block
          </button>

          <h4 className="font-semibold text-lg mt-8 mb-3 text-purple-800 border-b pb-2">
            Course Worth It Details
          </h4>
          <div className="mb-6">
            <RichTextField
              label="Main Introductory Description"
          ...
          {worthItTopics.map((topic, index) => (
            <div
              key={`wt-${index}`}
              className="mb-4 p-4 border border-purple-200 rounded-lg bg-white shadow-inner space-y-4"
            >
              <input
                type="text"
                value={topic.subHeading}
                onChange={(e) =>
                  handleObjectArrayChange(setWorthItTopics)(
                    index,
                    "subHeading",
                    e.target.value
                  )
                ...
                placeholder="Topic Heading (e.g., Flexibility)"
              />
              <RichTextField
                label="Topic Description"
                value={topic.description}
                onChange={(val) =>
                  handleObjectArrayChange(setWorthItTopics)(index, "description", val)
                }
                placeholder="Topic description…"
                minHeight="90px"
              />
              <div className="flex justify-end">
                {worthItTopics.length > 1 && (
                  <button
                    type="button"
                    onClick={() => handleObjectArrayRemove(setWorthItTopics)(index)}
                    className="w-full md:w-auto bg-red-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-red-600"
                  >
                    Remove
                  </button>
          ...
          <button
            type="button"
            onClick={handleObjectArrayAdd(setWorthItTopics, defaultWorthItTopic)}
            className="bg-purple-800 text-white px-4 py-2 rounded-lg text-sm hover:bg-purple-900 transition mt-2 shadow-md"
          >
            + Add Worth It Topic
          </button>
          ...
          <div className="mt-6 pt-4 border-t border-purple-200">
            <label className="block font-medium mb-1">
              Worth It Illustration Image
            </label>
            <input
              type="file"
              accept="image/*"
              onChange={(e) => {
                setWorthItImage(e.target.files[0]);
                setWorthItImage_old(null);
              }}
              className="w-full text-gray-700 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-purple-100 file:text-[#6A0DAD] hover:file:bg-purple-200 cursor-pointer"
            />
            {worthItImage_old && !worthItImage && (
              <p className="text-sm text-gray-500 mt-1">
                Current Image:{" "}
                <span className="font-semibold text-purple-700">Present</span>
              </p>
            )}
          </div>
        </div>
        ...
        {/* ============ 8. ADMISSION PROCESS ============ */}
        <div className="p-6 border border-emerald-400 rounded-xl bg-emerald-50 shadow-sm">
          <h3 className="font-bold text-xl mb-4 text-[#065F46]">
            8. 🎯 Admission Process
          </h3>
          {admissionProcess.map((step, i) => (
            <div
              key={i}
              className="p-4 mb-4 border border-emerald-200 rounded-lg bg-white shadow-inner space-y-3"
            >
              <div className="flex gap-2 items-center">
                <span className="bg-emerald-600 text-white w-8 h-8 rounded-full flex items-center justify-center font-bold">
                  {i + 1}
                </span>
                <input
                  type="text"
                  value={step.title}
                  onChange={(e) =>
                    handleObjectArrayChange(setAdmissionProcess)(
                      i,
                      "title",
                      e.target.value
                    )
                  ...
                  className="border rounded-lg p-2 flex-1"
                  placeholder="Title (e.g., Check Eligibility)"
                />
                {admissionProcess.length > 1 && (
                  <button
                    type="button"
                    onClick={() => handleObjectArrayRemove(setAdmissionProcess)(i)}
                    className="bg-red-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-red-600"
                  >
                    Remove
                  </button>
              ...
              <RichTextField
                ...
                onChange={(val) =>
                  handleObjectArrayChange(setAdmissionProcess)(i, "description", val)
                }
                placeholder="Description"
                minHeight="80px"
              />
          ...
          <button
            type="button"
            onClick={handleObjectArrayAdd(setAdmissionProcess, defaultAdmissionStep)}
            className="bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm hover:bg-emerald-800 transition mt-2 shadow-md"
          >
            + Add Admission Step
          </button>
        </div>
        ...
        {/* ============ 9. FEES ============ */}
        <div className="p-6 border border-red-400 rounded-xl bg-red-50 shadow-sm">
          <h3 className="font-bold text-xl mb-4 text-[#A00000]">
            9. Fee Structure Details
          </h3>

          <h4 className="font-semibold text-lg mb-3 text-red-800 border-b pb-2">
            Fee Sidebar Structure
          </h4>
          {feeSidebar.map((block, blockIndex) => (
            <div
              key={`fsb-${blockIndex}`}
              className="p-4 mb-4 border border-red-200 rounded-lg bg-white shadow-inner"
            >
              <h5 className="font-semibold text-md mb-2 flex justify-between items-center">
                <span className="text-red-700">Block Heading:</span>
                <input
                  ...
                  value={block.heading}
                  onChange={(e) =>
                    handleObjectArrayChange(setFeeSidebar)(
                      blockIndex,
                      "heading",
                      e.target.value
                    )
                ...
                {feeSidebar.length > 1 && (
                  <button
                    type="button"
                    onClick={() => handleObjectArrayRemove(setFeeSidebar)(blockIndex)}
                    className="text-red-600 text-sm hover:text-red-800"
                  >
                    Remove Block
                  </button>
          ...
          <button
            type="button"
            onClick={handleObjectArrayAdd(setFeeSidebar, defaultFeeSidebar)}
            className="bg-red-800 text-white px-4 py-2 rounded-lg text-sm hover:bg-red-900 transition mt-2 shadow-md"
          >
            + Add New Fee Sidebar Block
          </button>

          <h4 className="font-semibold text-lg mt-8 mb-3 text-red-800 border-b pb-2">
            Detailed Fees Tables
          </h4>
          {detailedFees.map((section, sectionIndex) => (
            <div
              key={`df-${sectionIndex}`}
              className="p-4 mb-4 border border-red-200 rounded-lg bg-white shadow-inner space-y-4"
            >
              <h5 className="font-semibold text-md mb-2 flex justify-between items-center">
                <span className="text-red-700">Section Heading:</span>
                <input
                  ...
                  onChange={(e) =>
                    handleObjectArrayChange(setDetailedFees)(
                      sectionIndex,
                      "heading",
                      e.target.value
                    )
                  ...
                  className="border rounded-lg p-1 w-1/2"
                  placeholder="e.g. Total Fees Breakdown"
                  required
                />
                {detailedFees.length > 1 && (
                  <button
                    type="button"
                    onClick={() =>
                      handleObjectArrayRemove(setDetailedFees)(sectionIndex)
                    }
                    className="text-red-600 text-sm hover:text-red-800"
                  >
                    Remove Section
              ...
              {section.table.map((row, rowIndex) => (
                <div
                  key={`dr-${rowIndex}`}
                  className="grid md:grid-cols-4 gap-2 mb-2 items-center"
                >
                  <input
                    type="text"
                    value={row.universityName}
                    onChange={(e) =>
                      handleDetailedFeesTableChange(
                        sectionIndex,
                        rowIndex,
                        "universityName",
                        e.target.value
                  ...
                  <input
                    ...
                    className="border rounded-lg p-2"
                    placeholder="Detailed Structure"
                  />
                  <div className="flex items-center justify-end">
                    {section.table.length > 1 && (
                      <button
                        type="button"
                        onClick={() =>
                          removeDetailedFeesTable(sectionIndex, rowIndex)
                        }
                        className="text-red-600 hover:text-red-800"
                      >
                        Remove
              ...
              <button
                type="button"
                onClick={() => addDetailedFeesTable(sectionIndex)}
                className="bg-red-600 text-white px-3 py-1 rounded-lg text-xs hover:bg-red-700 mt-2 shadow-sm"
              >
                + Add Fee Row
              </button>
          ...
          <button
            type="button"
            onClick={handleObjectArrayAdd(setDetailedFees, defaultDetailedFee)}
            className="bg-[#A00000] text-white px-4 py-2 rounded-lg text-sm hover:bg-red-700 transition mt-2 shadow-md"
          >
            + Add New Detailed Fees Section
          </button>
        </div>

        {/* ============ 10. EMI OPTIONS ============ */}
        <div className="p-6 border border-lime-400 rounded-xl bg-lime-50 shadow-sm">
          <h3 className="font-bold text-xl mb-4 text-[#3F6212]">
            10. 💳 EMI Options
          </h3>
          <label className="flex items-center gap-2 mb-4 cursor-pointer">
            <input
              type="checkbox"
              checked={emiOptions.enabled}
              onChange={(e) =>
                setEmiOptions((prev) => ({ ...prev, enabled: e.target.checked }))
              }
              className="w-4 h-4"
            />
            <span className="font-semibold">Enable EMI Options for this Course</span>
          </label>
          ...
          {emiOptions.enabled && (
            <div className="grid md:grid-cols-3 gap-3">
              <input
                type="text"
                placeholder="Min Monthly (₹3,000)"
                value={emiOptions.minMonthly}
                onChange={(e) =>
                  setEmiOptions((prev) => ({ ...prev, minMonthly: e.target.value }))
                }
                className="border rounded-lg p-2"
              />
              <input
                type="text"
                placeholder="Max Monthly (₹5,000)"
                value={emiOptions.maxMonthly}
                onChange={(e) =>
                  setEmiOptions((prev) => ({ ...prev, maxMonthly: e.target.value }))
                }
                className="border rounded-lg p-2"
              />
              <input
                type="text"
                placeholder="Tenures (6,12,24)"
                value={emiOptions.tenureMonths}
                onChange={(e) =>
                  setEmiOptions((prev) => ({
                    ...prev,
        ...
        {/* ============ 11. SCHOLARSHIPS ============ */}
        <div className="p-6 border border-amber-400 rounded-xl bg-amber-50 shadow-sm">
          <h3 className="font-bold text-xl mb-4 text-[#92400E]">
            11. 🎓 Scholarships
          </h3>
          {scholarships.map((item, i) => (
            <div
              key={i}
              className="p-4 mb-3 border border-amber-200 rounded-lg bg-white shadow-inner space-y-3"
            >
              <div className="flex gap-2">
                <input
                  type="text"
                  value={item.title}
                  onChange={(e) =>
                    handleObjectArrayChange(setScholarships)(i, "title", e.target.value)
                  }
                  className="border rounded-lg p-2 flex-1"
                  placeholder="Title (Merit Scholarship)"
                />
                <input
                  type="text"
                  value={item.discount}
                  onChange={(e) =>
                    handleObjectArrayChange(setScholarships)(
                      i,
                      "discount",
                      e.target.value
                    )
                  ...
                  placeholder="Discount (30%)"
                />
                {scholarships.length > 1 && (
                  <button
                    type="button"
                    onClick={() => handleObjectArrayRemove(setScholarships)(i)}
                    className="bg-red-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-red-600"
                  >
                    Remove
                  </button>
              ...
              <RichTextField
                ...
                value={item.description}
                onChange={(val) =>
                  handleObjectArrayChange(setScholarships)(i, "description", val)
                }
                placeholder="Description"
                minHeight="80px"
              />
          ...
          <button
            type="button"
            onClick={handleObjectArrayAdd(setScholarships, defaultScholarship)}
            className="bg-amber-700 text-white px-4 py-2 rounded-lg text-sm hover:bg-amber-800 transition mt-2 shadow-md"
          >
            + Add Scholarship
          </button>
        </div>
        ...
        <div className="p-6 border border-indigo-400 rounded-xl bg-indigo-50 shadow-sm">
          <h3 className="font-bold text-xl mb-4 text-[#4B0082]">
            12. Job Opportunities & Recruiters
          </h3>

          <h4 className="font-semibold text-lg mb-3 text-indigo-800 border-b pb-2">
            Job Opportunities
          </h4>
          {jobOpportunities.map((job, index) => (
            <div
              key={`jo-${index}`}
              className="mb-4 p-4 border border-indigo-200 rounded-lg bg-white shadow-inner space-y-4"
            >
              <div className="grid md:grid-cols-2 gap-4">
                <input
                  type="text"
                  value={job.heading}
                  onChange={(e) =>
                    handleObjectArrayChange(setJobOpportunities)(
                      index,
                      "heading",
                      e.target.value
                    )
              ...
              <RichTextField
                ...
                onChange={(val) =>
                  handleObjectArrayChange(setJobOpportunities)(index, "description", val)
                }
              ...
              <div className="flex justify-end">
                {jobOpportunities.length > 1 && (
                  <button
                    type="button"
                    onClick={() =>
                      handleObjectArrayRemove(setJobOpportunities)(index)
                    }
          ...
          <button
            type="button"
            onClick={handleObjectArrayAdd(setJobOpportunities, defaultJobOpportunity)}
            className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-indigo-700 transition mt-2 shadow-md"
          >
            + Add Job Role
          </button>

          <h4 className="font-semibold text-lg mt-8 mb-3 text-indigo-800 border-b pb-2">
            Top Recruiters
          </h4>
          {topRecruiters.map((recruiter, index) => (
            <div
              key={`tr-${index}`}
              className="grid md:grid-cols-3 gap-4 mb-4 p-4 border border-indigo-200 rounded-lg bg-white shadow-inner"
            >
              <input
                type="text"
                value={recruiter.companyName}
                onChange={(e) =>
                  handleObjectArrayChange(setTopRecruiters)(
                    index,
                    "companyName",
                    e.target.value
                  )
              ...
              <input
                type="text"
                value={recruiter.packageOffered}
                onChange={(e) =>
                  handleObjectArrayChange(setTopRecruiters)(
                    index,
                    "packageOffered",
              ...
              <div className="flex items-center justify-end">
                {topRecruiters.length > 1 && (
                  <button
                    type="button"
                    onClick={() => handleObjectArrayRemove(setTopRecruiters)(index)}
                    className="bg-red-500 text-white px-3 py-2 rounded-lg hover:bg-red-600 text-sm"
                  >
                    Remove
                  </button>
          ...
          <button
            type="button"
            onClick={handleObjectArrayAdd(setTopRecruiters, defaultRecruiter)}
            className="bg-indigo-800 text-white px-4 py-2 rounded-lg text-sm hover:bg-indigo-900 transition mt-2 shadow-md"
          >
            + Add Recruiter
          </button>
        </div>

        {/* ============ 13. PLACEMENT SUPPORT ============ */}
        <div className="p-6 border border-violet-400 rounded-xl bg-violet-50 shadow-sm">
          <h3 className="font-bold text-xl mb-4 text-[#5B21B6]">
            13. 💼 Placement Support
          </h3>
          <div className="p-4 mb-4 border border-violet-200 rounded-lg bg-white shadow-inner space-y-4">
            <RichTextField
              label="Description"
              value={placementSupport.description}
              onChange={(val) => handlePlacementChange("description", val)}
              placeholder="Description"
            />
            <h4 className="font-medium text-sm mt-3">Placement Stats:</h4>
            <div className="grid md:grid-cols-3 gap-3">
              <input
                type="text"
                placeholder="Placement Rate (85%)"
                value={placementSupport.stats.placementRate}
                onChange={(e) =>
                  handlePlacementStatChange("placementRate", e.target.value)
                }
                className="border rounded-lg p-2"
              />
              <input
                type="text"
                placeholder="Avg Package (₹4.5 LPA)"
                value={placementSupport.stats.avgPackage}
                onChange={(e) =>
                  handlePlacementStatChange("avgPackage", e.target.value)
                }
                className="border rounded-lg p-2"
              />
              <input
                type="text"
                placeholder="Highest Package (₹12 LPA)"
                value={placementSupport.stats.highestPackage}
                onChange={(e) =>
                  handlePlacementStatChange("highestPackage", e.target.value)
                }
                className="border rounded-lg p-2"
              />
            </div>
        ...
        {/* ============ 14. TESTIMONIALS ============ */}
        <div className="p-6 border border-rose-400 rounded-xl bg-rose-50 shadow-sm">
          <h3 className="font-bold text-xl mb-4 text-[#9F1239]">
            14. ⭐ Course Testimonials
          </h3>
          {courseTestimonials.map((item, i) => (
            <div
              key={i}
              className="p-4 mb-4 border border-rose-200 rounded-lg bg-white shadow-inner space-y-3"
            >
              <div className="flex gap-2">
                <input
                  type="text"
                ...
                />
                {courseTestimonials.length > 1 && (
                  <button
                    type="button"
                    onClick={() => handleObjectArrayRemove(setCourseTestimonials)(i)}
                    className="bg-red-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-red-600"
                  >
                    Remove
                  </button>
              ...
              <div className="grid md:grid-cols-3 gap-2">
                <input
                  type="text"
                  value={item.course}
                  onChange={(e) => handleTestimonialChange(i, "course", e.target.value)}
                  className="border rounded-lg p-2"
                  placeholder="Course (Online BCA)"
                />
                <input
                  type="text"
                  value={item.university}
                  onChange={(e) =>
                    handleTestimonialChange(i, "university", e.target.value)
                  }
                  className="border rounded-lg p-2"
              ...
              <div className="grid md:grid-cols-2 gap-2">
                <input
                  type="text"
                  value={item.year}
                  onChange={(e) => handleTestimonialChange(i, "year", e.target.value)}
                  className="border rounded-lg p-2"
                ...
                <div>
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(e) => handleTestimonialImage(i, e.target.files[0])}
                    className="border rounded-lg p-2 w-full"
                  />
                  {item.image_old && !item.image && (
                    <p className="text-xs text-gray-500 mt-1">
                      Current Image: <span className="text-rose-700">Present</span>
                    </p>
                  )}
                </div>
              </div>
              <RichTextField
                label="Review"
                value={item.review}
                onChange={(val) => handleTestimonialChange(i, "review", val)}
                placeholder="Review"
                minHeight="80px"
              />
            </div>
          ...
          <button
            type="button"
            onClick={handleObjectArrayAdd(setCourseTestimonials, defaultTestimonial)}
            className="bg-rose-700 text-white px-4 py-2 rounded-lg text-sm hover:bg-rose-800 transition mt-2 shadow-md"
          >
            + Add Testimonial
          </button>
        </div>

        {/* ============ 15. CAREER VIDYA BENEFITS ============ */}
        <div className="p-6 border border-cyan-400 rounded-xl bg-cyan-50 shadow-sm">
          <h3 className="font-bold text-xl mb-4 text-[#006064]">
            15. Career Vidya Benefits
          </h3>
          {careerBenefits.map((item, index) => (
            <div
              key={`cb-${index}`}
              className="p-4 mb-4 border border-cyan-200 rounded-lg bg-white shadow-inner space-y-3"
            >
              <div className="flex justify-between items-center gap-2">
                <input
                  type="text"
                  value={item.title}
                  onChange={(e) =>
                    handleCareerBenefitChange(index, "title", e.target.value)
                  }
                  className="border rounded-lg p-2 flex-1"
                  placeholder="Benefit Title (e.g., No Cost EMI)"
                />
                {careerBenefits.length > 1 && (
                  <button
                    type="button"
                    onClick={() => handleObjectArrayRemove(setCareerBenefits)(index)}
                    className="bg-red-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-red-600 whitespace-nowrap"
                  >
                    Remove
                  </button>
                )}
              ...
              <RichTextField
                ...
                value={item.description}
                onChange={(val) =>
                  handleCareerBenefitChange(index, "description", val)
                }
                placeholder="Describe this benefit…"
              />
            </div>
          ...
          <button
            type="button"
            onClick={handleObjectArrayAdd(setCareerBenefits, defaultCareerBenefit)}
            className="bg-cyan-700 text-white px-4 py-2 rounded-lg text-sm hover:bg-cyan-800 transition mt-2 shadow-md"
          >
            + Add Career Benefit
          </button>
        </div>
        ...
        {/* ============ 16. FAQ ============ */}
        <div className="p-6 border border-orange-400 rounded-xl bg-orange-50 shadow-sm">
          <h3 className="font-bold text-xl mb-4 text-[#9A3412]">
            16. Frequently Asked Questions
          </h3>
          {faqs.map((item, index) => (
            <div
              key={`faq-${index}`}
              className="p-4 mb-4 border border-orange-200 rounded-lg bg-white shadow-inner space-y-3"
            >
              <div className="flex justify-between items-center gap-2">
                <input
                  type="text"
                ...
                {faqs.length > 1 && (
                  <button
                    type="button"
                    onClick={() => handleObjectArrayRemove(setFaqs)(index)}
                    className="bg-red-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-red-600 whitespace-nowrap"
                  >
                    Remove
                  </button>
          ...
          <button
            type="button"
            onClick={handleObjectArrayAdd(setFaqs, defaultFaq)}
            className="bg-orange-700 text-white px-4 py-2 rounded-lg text-sm hover:bg-orange-800 transition mt-2 shadow-md"
          >
            + Add FAQ
          </button>
        </div>
        ...
        <div className="pt-8 border-t border-gray-200">
          {message && (
            <p
              className={`text-center font-semibold mb-4 text-sm ${
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full py-3 rounded-lg text-lg font-bold text-white transition shadow-lg ${
              loading
                ? "bg-gray-500 cursor-not-allowed"
                : "bg-[#002D62] hover:bg-blue-800"
            }`}
          >
            {loading ? "Updating..." : "💾 Update Course Data"}
          </button>
