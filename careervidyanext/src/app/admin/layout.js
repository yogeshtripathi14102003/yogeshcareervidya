...
const AdminShell = ({ children }) => {
  ...
  // ----------------------------------------------------------------
  // MENU STRUCTURE
  // Icons ab har item ke actual meaning ke hisaab se match kiye gaye hain
  // (pehle bahut jagah generic Users/Lock/MessageSquare reuse ho raha tha).
  // ----------------------------------------------------------------
  const menuItems = [
    { href: "/admin", label: "Dashboard", icon: LayoutDashboard },
    { href: "/admin/visitors", label: "Visitors", icon: Users },

    {
      ...
      children: [
        { href: "/admin/Getalluser", label: "All Students", icon: Users },
        { href: "/admin/getquery", label: "Get Queries", icon: MessageSquare },
        { href: "/admin/applyadmission", label: "applyadmission", icon: ClipboardCheck },
        { href: "/admin/add-subsid", label: "add-subsid", icon: FileText },
      ],
    },
    {
      id: "courseAndUniversity",
      ...
      children: [
        ...
        { href: "/admin/addblog", label: "addblog", icon: Newspaper },
        { href: "/admin/getbloglist", label: "getbloglist", icon: FileText },
        { href: "/admin/placestudent", label: "Placed Students", icon: Award },
        { href: "/admin/addteam", label: "Add Team", icon: UserPlus },
        { href: "/admin/manageteam", label: "ourteam", icon: Users },
        { href: "/admin/bannerlist", label: "Banners", icon: ImageIcon },
        { href: "/admin/state", label: "state", icon: MapPin },
        { href: "/admin/newsletter", label: "Newsletter", icon: Mail },
      ],
    },

    { href: "/admin/job", label: "Job Posts", icon: Briefcase },
    { href: "/admin/getresume", label: "Applications", icon: FileText },
    { href: "/admin/VideoPage", label: "Videos", icon: Video },
    ...
    // ---------------- Employee Management group ----------------
    {
      ...
      icon: FolderKanban,
      children: [
        { href: "/admin/empmanagement/upload", label: "Employee Upload", icon: Upload, id: "employeeupload" },
        { href: "/admin/empmanagement/dashbord", label: "Employee Dashboard", icon: LayoutDashboard, id: "dashbord" },
        { href: "/admin/empmanagement/alerts", label: "Employee Alerts", icon: Bell, id: "employeealerts" },
        { href: "/admin/empmanagement/emp", label: "Employee List", icon: ListChecks, id: "employeelist" },
      ],
    },

    {
      ...
      icon: Lock,
      children: [
        { href: "/admin/SecuritySettings", label: "Security Settings", icon: Settings2, id: "securitysettings" },
        { href: "/admin/Security", label: "Security", icon: Lock },
      ],
    },

    { href: "/admin/slotsquery", label: "Slot Management", icon: Calendar, id: "slotmanagement" },
  ];
  ...
  return (
    ...
    >
      {/* 🛠️ MASTER INJECTION: sub-pages ke default components (table, tr, td, card) ka color force badal dega */}
      {darkMode && (
        <style
      ...
      {/* HEADER */}
      <header
        className={`fixed top-0 left-0 right-0 h-12 shadow flex items-center justify-between px-3 z-50 transition-colors duration-200 ${
          darkMode ? "bg-gray-800 border-b border-gray-700 text-white" : "bg-white text-gray-900"
        }`}
      >
        <div className="flex items-center gap-2">
          <button
            onClick={toggleSidebar}
            className={`md:hidden p-1.5 rounded ${darkMode ? "hover:bg-gray-700 text-white" : "hover:bg-gray-200"}`}
          >
            <Menu size={18} />
          </button>
          <Image
            ...
            width={90}
            height={80}
            className={`${darkMode ? "brightness-110 contrast-125" : ""}`}
          />
          <h1 className="text-sm font-semibold uppercase">{userRole} Portal</h1>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="text-xs text-red-500 px-2 py-1 rounded flex items-center gap-1 hover:bg-red-50 dark:hover:bg-red-950/20"
          >
            <LogOut size={14} /> Logout
          </button>
        </div>
      ...
      {/* SIDEBAR — navy blue theme with dotted texture, always this palette
          regardless of the light/dark toggle (which only affects main content) */}
      <aside
        className={`fixed top-12 left-0 h-full w-60 transition-transform z-40 cv-sidebar ${
      ...
      >
        <style>{`
          ...
          .cv-nav-link {
            color: #c7d3f5; /* soft, readable blue-white — not dim gray */
          }
          .cv-nav-link:hover {
          ...
          .cv-nav-icon { color: #7dd3fc; }
          .cv-nav-link.active .cv-nav-icon { color: #38bdf8; }
          .cv-group-label { color: #eaf0ff; }
        `}</style>

        <nav className="px-2 py-3 overflow-y-auto h-[calc(100vh-48px)]">
          <ul className="space-y-0.5 pb-20">
            {filteredMenu.map((item) => {
              // ---------- Group item (dropdown) ----------
              if (item.children) {
                ...
                return (
                  <li key={item.id}>
                    <button
                      type="button"
                      onClick={() => toggleGroup(item.id)}
                      className={`cv-nav-link w-full flex items-center justify-between gap-2 px-2.5 py-2 rounded-lg text-[12.5px] transition-colors ${
                        isChildActive ? "active" : ""
                      }`}
                    >
                      <span className="flex items-center gap-2.5">
                        <GroupIcon size={16} className="cv-nav-icon flex-shrink-0" />
                        <span className="cv-group-label">{item.label}</span>
                      </span>
                      <ChevronDown
                        size={14}
                    ...
                    >
                      {item.children.map((child) => (
                        <li key={child.href}>
                          <Link
                            href={child.href}
                            className={`cv-nav-link flex items-center gap-2.5 pl-8 pr-2.5 py-1.5 rounded-lg text-[12px] transition-colors ${
                              pathname === child.href ? "active" : ""
                            }`}
                          >
                            <child.icon size={14} className="cv-nav-icon flex-shrink-0" />
                            <span>{child.label}</span>
                          </Link>
                        </li>
                      ))}
              ...
              return (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className={`cv-nav-link flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-[12.5px] transition-colors ${
                      pathname === item.href ? "active" : ""
                    }`}
                  >
                    <Icon size={16} className="cv-nav-icon flex-shrink-0" />
                    <span>{item.label}</span>
                  </Link>
                </li>
              );
      ...
      {/* MAIN CONTAINER */}
      <main
        className={`flex-1 pt-14 md:ml-60 px-4 py-3 overflow-y-auto subpixel-antialiased transition-colors duration-200 ${
