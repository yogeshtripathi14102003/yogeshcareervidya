    ...
    <>
      {showPopup && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg rounded-2xl shadow-2xl bg-white overflow-hidden border-2 border-[#000080]">
            <button onClick={() => setShowPopup(false)} className="absolute top-4 right-4 text-white z-20 hover:scale-110 transition-transform">
              <X size={24} />
            </button>

            {!showSignup ? (
              /* --- DARK BLUE THEME SECTION --- */
              <div className="relative p-8 text-center bg-[#000080] min-h-[400px] flex flex-col justify-center items-center">
                
                {/* Decorative Pattern Layer */}
                <div className="absolute inset-0 opacity-10 pointer-events-none overflow-hidden">
                  <div className="absolute top-0 right-0 w-64 h-64 bg-white rounded-full -mr-20 -mt-20 blur-3xl"></div>
                  <div className="absolute bottom-0 left-0 w-64 h-64 bg-blue-400 rounded-full -ml-20 -mb-20 blur-3xl"></div>
                </div>

                <div className="relative z-10">
                  <h2 className="text-4xl font-black italic text-white drop-shadow-md">GetAdmission</h2>
                  <p className="text-sm mt-2 font-bold text-blue-200 tracking-[0.3em] uppercase">Empowering Your Future</p>
                  
                  <div className="mt-8 flex flex-col items-center">
                    <div className="flex items-baseline gap-1">
                      <span className="text-8xl font-black text-white leading-none tracking-tight">{offer.discountPercentage}</span>
                      <span className="text-4xl font-bold text-blue-300">% OFF</span>
                    </div>
                    <p className="text-[11px] font-black text-white mt-4 bg-white/10 px-4 py-1.5 rounded-full border border-white/20 tracking-wider uppercase">
                      Exclusive Academic Subsidy
                    </p>
                  </div>

                  <button
                    onClick={handleGetClick}
                    className="bg-white text-[#000080] py-4 px-12 rounded-full text-xl font-bold flex items-center gap-2 mx-auto mt-10 shadow-2xl transition-all hover:bg-blue-50 hover:scale-105 active:scale-95"
                  >
                    GET NOW <Zap fill="currentColor" size={20} />
                  </button>
              ...
              <div className="p-5 max-h-[85vh] overflow-y-auto bg-white">
                <div className="flex items-center gap-3 mb-4 border-b pb-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-[#000080]">#VidyaHaiTohSuccessHai</p>
                    <p className="text-[11px] text-gray-500 font-medium">Student's Trusted Education Guidance Platform</p>
                  </div>
                </div>
                ...
                <form className="space-y-1">
                  ...
                  <button
                    ...
                    onClick={handleSubmit}
                    disabled={loading}
                    className="w-full py-3 mt-2 rounded-md text-white font-bold bg-[#000080] hover:bg-blue-900 transition-colors shadow-lg shadow-blue-900/20"
                  >
                    {loading ? "Processing..." : otpSent ? "Verify & Register" : "Send OTP to Apply"}
                  </button>
                  <p className="text-[10px] text-center text-gray-400 mt-2 italic">Secured by GetAdmission Academic Council</p>
      ...
      {showSuccessPopup && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-sm w-full p-10 text-center border-t-8 border-[#000080]">
            <div className="text-6xl mb-4">🎓</div>
            <h3 className="text-2xl font-bold text-[#000080] mb-2">Congratulations!</h3>
