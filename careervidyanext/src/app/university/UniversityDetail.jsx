...
// ✅ FIX: BASE_URL was commented out but still used below — this crashed
// getImagePath() the moment any relative (non-http) image path showed up.
    ...
    return (
        <>
            <div className="bg-white text-black min-h-screen w-full">
                ...
                <div className="max-w-7xl mx-auto px-6 py-12 space-y-12">
                    {/* ✅ H2 sections under the page H1 — consistent hierarchy */}
                    <div ref={overviewRef}><div className="bg-white border border-gray-200 rounded-xl p-8 shadow-sm"><h2 className="text-[#0056D2] text-4xl font-bold mb-8">Overview</h2><div className="text-[#4A4A4A] text-[17px] leading-[1.8] space-y-6" dangerouslySetInnerHTML={{ __html: data.description || "No description available." }} /></div></div>
                    <div ref={certificateRef}><UniversityCertificate slug={data.slug} /></div>
                    <div ref={coursesRef} className="mb-16 shadow-lg rounded-2xl overflow-hidden border border-gray-100">
                        {/* ✅ Changed h3 → h2 to keep heading levels sequential
                            (H1 page title → H2 section headings, no skipped levels) */}
