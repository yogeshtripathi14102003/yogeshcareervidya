...
// ✅ UPDATED: historyByDate (backend grouped) ko prefer karta hai
const getLogsArray = (lead) => {
  if (!lead) return [];

  // ✅ Priority 1: historyByDate (backend se grouped, IST-aware)
  if (Array.isArray(lead.historyByDate) && lead.historyByDate.length > 0) {
    const flat = [];
    lead.historyByDate.forEach((day) => {
      (day.entries || []).forEach((entry) => {
        flat.push({
          ...
          status: entry.status || "",
          date: entry.date,       // UTC
          dateIST: entry.dateIST, // formatted IST
        });
      });
    });
    return flat;
...
// ✅ UPDATED: date field priority (historyByDate entries me `date` hota hai)
const getLogDate = (log, leadUpdatedAt) => {
  if (typeof log !== "object" || log === null) return leadUpdatedAt || null;
  return log.date || log.createdAt || log.timestamp || log.updatedAt || leadUpdatedAt || null;
};

// ✅ UPDATED: remark text nikaalo
const getLogRemark = (log) => {
  if (typeof log === "string") return log;
  if (typeof log !== "object" || log === null) return "—";
  return log.remark || log.text || log.comment || log.remarks ||
    log.note || log.message || log.description || "—";
};
...
// ✅ NEW: log ka formatted IST string (dateIST prefer karo)
const getLogDateDisplay = (log, leadUpdatedAt) => {
  if (log?.dateIST) return log.dateIST;
  return fmtDate(getLogDate(log, leadUpdatedAt));
};
...
const RemarkActivityPage = ({ isAdmin = false }) => {
  ...
  const fetchLeads = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get("/api/v1/counselor-leads", { params });
      if (res.data.success) {
        // ✅ Backend ab remarkCount + historyByDate bhej raha hai
        setLeads(res.data.data || []);
      }
    } catch (err) {
      console.error("API error:", err);
              ...
              <table className="w-full text-[11px] min-w-[800px]">
                ...
                <tbody className="divide-y divide-slate-100 bg-white">
                  {paginatedLeads.map((lead, idx) => {
                    ...
                    // ✅ Total count: backend remarkCount prefer karo (accurate)
                    const totalLogsCount =
                      typeof lead.remarkCount === "number"
                        ? lead.remarkCount
                        : logList.length > 0
                        ? logList.length
                        : hasRootRemark
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-3">
                          <div className="flex flex-col gap-1 max-h-28 overflow-y-auto pr-1">
                            {logsInScope.length > 0 ? (
                              ...
                              [...logsInScope].reverse().map((log, i) => {
                                const remarkText = getLogRemark(log);
                                const logDateDisplay = getLogDateDisplay(log, lead.updatedAt);
                                return (
                                  <div key={i} className="bg-slate-50 border rounded p-1.5 border-slate-200">
                                    {logDateDisplay && (
                                      <p className="text-[8px] text-slate-400 font-black mb-0.5">
                              ...
                              <div className="bg-emerald-50/50 border border-emerald-100 rounded p-1.5">
                                <p className="text-[8px] text-emerald-600 font-black mb-0.5">
                                  ⏱️ {lead.updatedAtIST || fmtDate(lead.updatedAt)}
                                </p>
                                <p className="text-[10px] text-slate-800 font-bold">
                                  {rootRemark}
                                </p>
                        ...
                        <td className="p-3 text-slate-500 font-semibold whitespace-nowrap">
                          {lead.updatedAtIST || fmtDate(lead.updatedAt || lead.createdAt)}
                        </td>
                      </tr>
                    );
                  })}
