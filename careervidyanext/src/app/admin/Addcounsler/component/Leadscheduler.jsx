      ...
      <div className="max-w-7xl mx-auto">
        ...
        {leads.length > 0 && (
          <div className="flex justify-end gap-3 mb-5">
            <button onClick={autoDistribute} className="bg-indigo-600 text-white px-4 py-2 rounded flex gap-1">
              <Shuffle size={16} /> Auto
            </button>
            <button onClick={assignByCount} className="bg-orange-600 text-white px-4 py-2 rounded flex gap-1">🎯 By Count</button>
            <button onClick={saveDistribution} disabled={loading} className="bg-green-600 text-white px-4 py-2 rounded flex gap-1">
              <Save size={16} /> Assign
            </button>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div className="bg-white w-full max-w-xl rounded p-5 max-h-[90vh] overflow-hidden flex flex-col">
              <div className="flex justify-between mb-3">
                ...
                <button onClick={() => setViewCounselor(null)} className="text-red-500 font-bold">✕</button>
              </div>
              <div className="flex gap-2 mb-3 items-center">
                <label className="text-sm">From:</label>
                <input type="date" value={filterDates.from} onChange={(e) => setFilterDates(p => ({ ...p, from: e.target.value }))} className="border px-2 py-1 rounded" />
                <label className="text-sm">To:</label>
                <input type="date" value={filterDates.to} onChange={(e) => setFilterDates(p => ({ ...p, to: e.target.value }))} className="border px-2 py-1 rounded" />
              </div>
              <div className="flex-1 overflow-y-auto border rounded p-2">
                {filteredLeads.map((l) => (
                  <div key={l._id} className="border-b p-2">
                    <p className="font-medium">{l.name}</p>
                    <p className="text-xs text-gray-500">{l.phone} | {l.course} | {new Date(l.createdAt).toLocaleDateString()}</p>
                  </div>
                ))}
              </div>
            </div>
