...
const LeadRow = ({ lead, onSave }) => {
  ...
  const [localDate, setLocalDate] = useState(formatForInput(lead.followUpDate));
  const [showAdmission, setShowAdmission] = useState(false);
  // Track remark optimistically after save
  const [savedRemark, setSavedRemark] = useState(lead.remark || "");

  const phoneNumber = lead.phone || lead.mobile || lead.contactNo;
  const hasChange = localStatus !== lead.status || localRemark.trim() !== "" || localDate !== formatForInput(lead.followUpDate);

  // Use savedRemark (optimistic) for green check so it turns green instantly after save
  ...
  const handleUpdate = () => {
    ...
    // 2. REMARK HISTORY — ISO timestamp for reliable parsing
    ...
    // 3. Optimistically update savedRemark so green shows instantly
