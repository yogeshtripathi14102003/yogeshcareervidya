...
const UniversityTable = ({ courseName = "Online Courses" }) => {
  ...
  // Priority universities (always top)
  const prioritySlugs = [
  ...
  useEffect(() => {
    const fetchUniversities = async () => {
      try {
        ...
        const allUniversities = res.data?.data || [];

        // Get priority universities first
        const priorityUnis = prioritySlugs
          .map(slug => allUniversities.find(u => u.slug === slug))
          .filter(Boolean);

        // Get the rest of universities excluding priority ones
        const remainingUnis = allUniversities.filter(
          u => !prioritySlugs.includes(u.slug)
        );

        // Merge priority + remaining, but only keep first 10
        const finalList = [...priorityUnis, ...remainingUnis].slice(0, 10);
          ...
          <table className="w-full border-collapse bg-white table-fixed">
            ...
            <tbody className="bg-white">
              {universities.map((uni, index) => {
                ...
                // Make the 6 priority universities always clickable
                const isClickable = prioritySlugs.includes(uni.slug);
