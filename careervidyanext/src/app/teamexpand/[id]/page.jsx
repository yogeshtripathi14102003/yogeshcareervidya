...
// ─── Security helpers (same as before) ─────────────────────────────────────
function sanitizePhone(raw) {
  if (typeof raw !== "string") return "";
  return raw.replace(/[^\d+]/g, "").slice(0, 15);
}
...
// Server-side fetch — runs BEFORE any HTML is sent, so both crawlers and
// real users get the actual mentor data baked into the first response.
// This replaces the old useEffect-based client fetch entirely.
async function getMentorData(id) {
  try {
    ...
    const mentor = mentorRes.data.data || mentorRes.data;
    const reviews = reviewsRes.data.reviews || [];
    if (!mentor || !mentor._id) return null;
...
// ✅ Title/description built from REAL mentor data, unique per id.
// Before: every /teamexpand/{id} page had the same static title —
// that's what caused the "Page Titles: Duplicate" warning on 38 URLs.
export async function generateMetadata({ params }) {
  const { id } = await params;
  const data = await getMentorData(id);

  if (!data) {
    return {
      title: "Counsellor Not Found | CareerVidya",
      robots: { index: false, follow: true },
    };
  }

  const { mentor } = data;
  const name = mentor.name || "Career Counsellor";
  const designation = mentor.designation || "Career Counsellor";
  const description = mentor.description
    ? mentor.description.slice(0, 155)
    : `${name} is a ${designation} at CareerVidya with ${mentor.experience || 0}+ years of experience guiding students toward the right career.`;

  return {
    title: `${name} - ${designation} | CareerVidya`,
    description,
    alternates: {
...
// ✅ Single async Server Component — no separate client file needed.
// Fetches mentor + reviews, then renders everything directly. The <h1>,
// description, fees, education, reviews — all of it exists in the real
// server-rendered HTML now, so "H1: Missing" and thin-content warnings
// go away. Header/Footer/ReviewForm can still be Client Components
// internally (they can be "use client" in their own files) — a Server
// Component is always allowed to render Client Components as children.
export default async function TeamDetailPage({ params }) {
  const { id } = await params;
  const data = await getMentorData(id);

  // Real 404 status instead of a 200 "not found" message — better for SEO.
  if (!data) notFound();
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="max-w-6xl mx-auto py-10 px-4">
        {/* Plain <Link> instead of a router.back() button — works fine
            inside a Server Component, no client hook required. */}
        <Link
          href="/teamexpand"
          className="mb-6 inline-flex items-center gap-2 text-gray-600 hover:text-blue-600 transition-colors font-medium group focus:outline-none focus:ring-2 focus:ring-blue-300 rounded"
        >
          <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" aria-hidden="true" />
          Back to Experts
        </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
              ...
              <div className="grid lg:grid-cols-2 gap-10">
                <div>
                  {/* NOTE: no onSuccess callback passed from here — this is
                      a Server Component and can't pass functions to a
                      Client Component. Add a `router.refresh()` call
                      inside ReviewForm's OWN success handler (it's already
                      "use client", so it has access to useRouter) — that
                      re-runs this Server Component and shows the new
                      review immediately without needing a callback prop. */}
                  <ReviewForm counsellorId={mentor._id} />
                </div>
