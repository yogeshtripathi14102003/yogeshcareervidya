...
// ✅ Courses arrive as props (fetched server-side in Footer.jsx) — no more
// client-side api.get() + loading window where links didn't exist yet.
//
// ✅ REMOVED the old `if (!mounted) return <footer .../>` guard that hid
// the ENTIRE footer (Quick Links, all course links, everything) until
// after JS hydration. That guard existed only to safely check
// window.innerWidth for the mobile banner — it was accidentally hiding
// this page's single biggest block of internal links from any crawler
// that doesn't fully execute JS. The footer content now always renders;
// only the TopOfferBanner (which has no SEO value) waits for the client
// mobile check.
  ...
  return (
    <footer className="bg-[#000d2d] text-white pt-16 pb-8 font-sans">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 mb-12">
          ...
          <div>
            ...
            <ul className="space-y-3 mt-4 text-gray-300 text-sm">
              ...
              <li><Link href="/teamexpand" className="hover:text-white transition">Our Team</Link></li>
              <li><Link href="/Counslerlogin" className="hover:text-white transition">Counselor Login</Link></li>
        ...
        {/* MIDDLE ROW: Dynamic Courses — now server-rendered, always in the DOM */}
      ...
      {/* Banner is decorative/promotional only — fine to wait for client mount */}
      {bannerReady && !isMobile && <TopOfferBanner />}
    </footer>
  );
}