...
// // ✅ No hardcoded fallback — fail loud if API_URL isn't set, same as
// // serverFetch.js, instead of silently building broken image URLs.
...
//   if (!BASE_URL) {
//     console.error(
//       "LogoSlider: API_URL environment variable is not set — cannot build image URL."
//     );
//     return "";
//   }
...
//   try {
//     const { ok, data: json } = await serverFetch("/api/v1/ourstudent", {
//       next: { revalidate: 300 },
//     });
...
//   // ✅ Skip logos with no resolvable URL (e.g. BASE_URL missing) so
//   // <Image> never gets an empty src and crashes the render.
//   logos = logos.filter((logo) => logo.logo);
    ...
    if (ok) {
      ...
      data.forEach((student) => {
        if (student.companyLogo && !seen.has(student.companyLogo)) {
          logos.push({
            logo: getFullUrl(student.companyLogo),
            // ✅ trim first — a whitespace-only string like " " is truthy,
            // so `student.company || fallback` would never fire without this,
            // and the page would render alt=" " (blank alt text).
          ...
          <div className="flex gap-6 animate-logo-scroll whitespace-nowrap">
            {doubled.map((logo, index) => (
              ...
              >
                <Image
                  src={logo.logo}
                  // ✅ now safe: logo.company is either a real trimmed name
                  // or "" (falsy), so the fallback always kicks in correctly.
                  alt={logo.company || `Company Logo ${index + 1}`}
                  width={120}
                  height={60}
                  className="object-contain max-w-full max-h-full"
