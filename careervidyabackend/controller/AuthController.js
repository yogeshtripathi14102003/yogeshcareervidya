...
import { autoAssignLead, getAssignmentConfig } from "../utilities/leadAssignmentEngine.js";
import { ADMITTED_STATUS, LOST_STATUSES } from "../constant/leadStatus.js";
import { paginatedSearch } from "../utilities/queryHelper.js"// NEW: generic pagination/search helper
...
/* -------------------- GET ALL STUDENTS --------------------
 * UPDATED: now uses the generic paginatedSearch() helper
 * (utilities/queryHelper.js) instead of hand-rolled pagination/search
 * logic. Behavior is the same as before, plus:
 *  - search input is regex-escaped (prevents regex-injection / 500s
 *    if someone searches for something like "a(b")
 *  - limit cap raised from 100 -> 500 so the frontend can request
 *    bigger pages when it genuinely needs to, without removing
 *    pagination altogether
 * Query params supported: page, limit, search
 *   GET /api/students?page=1&limit=20
 *   GET /api/students?page=1&limit=20&search=rahul
 */
export const getAllStudents = async (req, res) => {
  try {
    const result = await paginatedSearch(
      Student,
      { role: { $ne: "admin" } },        // base filter (unchanged)
      req.query,                          // page, limit, search
      ["name", "email", "mobileNumber"],  // fields to search across
      "-password -__v"                    // same exclusion as before
    );
