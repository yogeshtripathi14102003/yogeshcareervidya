...
export default function TeamSliderClient({ team = [] }) {
  const visibleTeam = team.slice(0, 10);
  ...
  const jsonLd = {
    ...
    description:
      "Meet our expert career counsellors at CareerVidya with years of experience in guiding students.",
    numberOfItems: visibleTeam.length,
    itemListElement: visibleTeam.map((member, i) => ({
      "@type": "ListItem",
      position: i + 1,
      item: {
        "@type": "Person",
            ...
            <div className="flex-grow overflow-hidden px-2">
              ...
              >
                {visibleTeam.map((member, idx) => {
                  const displayName = member.name || "Career Counselor";
                  const years = member.experience || 0;

                  return (
                    <SwiperSlide key={member._id ?? idx} className="!h-auto flex">
                      <Link
                        href={`/teamexpand?id=${member._id}`}
                        className="group relative flex w-full h-full flex-col overflow-hidden rounded-2xl bg-white shadow-[0_8px_30px_rgba(0,0,0,0.12)] transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(0,26,65,0.25)]"
                        aria-label={`View profile of ${displayName}`}
                      >
                        {/* 🌟 IMAGE CONTAINER: Increased height and changed object positioning */}
                        <div className="relative h-64 md:h-72 w-full flex-shrink-0 overflow-hidden bg-[#eef4ff]">
                          <Image
                            src={member.imageUrl}
                            alt={`${displayName} - ${member.designation || "Expert Counselor"} at CareerVidya`}
                            fill
                          ...
                          {/* Experience Tab */}
                          <div className="absolute left-0 top-6 flex -translate-x-1 items-center gap-1 rounded-r-lg bg-[#05347f] py-2 pl-3 pr-2 text-[11px] font-bold uppercase tracking-wide text-white shadow-sm z-10">
                        ...
                        {/* Info Block - Reduced overlap so photo bottom part shows properly */}
                        <div className="relative -mt-2 flex flex-grow flex-col items-start rounded-t-2xl bg-white px-6 pt-5">
                          <h3 className="mb-1 line-clamp-1 text-lg font-bold text-[#001a41]">
                            {displayName}
                          </h3>
                          <p className="mb-5 text-sm font-semibold uppercase tracking-wide text-[#3498db]">
                            {member.designation || "Expert Counselor"}
                          </p>
                        </div>

                        {/* Bottom Action Bar */}
                        <div className="flex w-full flex-shrink-0 items-center justify-center gap-2 bg-[#05347f] py-3.5 text-sm font-bold text-white transition-colors duration-300 group-hover:bg-[#c15304]">
                          Consult Now
