...
/* =====================================================
   ✅ NEW HELPERS — IST-aware date boundaries
===================================================== */
const IST_OFFSET_MS = 5.5 * 60 * 60 * 1000; // +05:30 in ms

/**
 * Current time ki IST date ko "naive" local Date me convert karo.
 * Isse hum IST ke hisaab se setHours() kar sakte hain.
 */
const nowInISTAsLocal = () => {
  const now = new Date();
  return new Date(now.getTime() + IST_OFFSET_MS);
};

/**
 * IST midnight ko UTC me convert karo (MongoDB me store karne ke liye).
 */
const toUTCFromISTLocal = (istLocalDate) => {
  return new Date(istLocalDate.getTime() - IST_OFFSET_MS);
};

/**
 * Aaj ki IST midnight (UTC me).
 */
const getISTStartOfDay = () => {
  const ist = nowInISTAsLocal();
  ist.setHours(0, 0, 0, 0);
  return toUTCFromISTLocal(ist);
};

/**
 * Is week ki IST Monday midnight (UTC me).
 * NOTE: Sunday ko week ka pehla din maana (getDay() === 0)
 */
const getISTStartOfWeek = () => {
  const ist = nowInISTAsLocal();
  ist.setDate(ist.getDate() - ist.getDay());
  ist.setHours(0, 0, 0, 0);
  return toUTCFromISTLocal(ist);
};

/**
 * Is month ki IST 1st midnight (UTC me).
 */
...
export const createCounselor = async (req, res) => {
  try {
    const {
      name, email, userid, password, phone, pan, aadhar, dob, doj, address, status, leadLimit,
    } = req.body;
    ...
    const exists = await Counselor.findOne({ $or: [{ email }, { userid }] });
    if (exists) {
    ...
    const counselor = await Counselor.create({
      name, email, userid, password: hashPassword, phone, pan, aadhar, dob, doj, address,
      status: status || "active",
      leadLimit: leadLimit != null && leadLimit !== "" ? Number(leadLimit) : null,
    });
...
export const getAllCounselors = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, total: counselors.length, data: counselors });
  } catch (error) {
    console.error("Fetch All Error:", error);
    res.status(500).json({ success: false, message: "Server Error" });
  }
...
export const getCounselorById = async (req, res) => {
  try {
    ...
    console.error("Fetch One Error:", error);
    res.status(500).json({ success: false, message: "Invalid ID / Server Error" });
  }
};
...
export const updateCounselor = async (req, res) => {
  try {
    ...
    const counselor = await Counselor.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true,
    }).select("-password");
    ...
    res.status(200).json({ success: true, message: "Counselor Updated", data: counselor });
  } catch (error) {
    console.error("Update Error:", error);
    res.status(500).json({ success: false, message: error.message || "Server Error" });
  }
...
/* ✅ UPDATED: getCounselorAnalytics
   - pendingCalls ab FULL POOL se count hoga (date filter ka asar nahi)
   - overdueCalls + freshUncontacted bhi add kiye
   - Performance metrics date range ke hisaab se rahenge (totalLeads, admissions, etc.)
*/
export const getCounselorAnalytics = async (req, res) => {
  try {
    const targetId = req.params.counselorId || String(req.user._id);
    ...
    // ---- Lead-derived stats (date range ke hisaab se) ----
    const leadFilterForRange = { assignedTo: targetId };
    if (Object.keys(dateFilter).length) leadFilterForRange.createdAt = dateFilter;

    const leadsInRange = await Lead.find(leadFilterForRange)
      .select("status createdAt firstResponseAt resolvedAt followUpHistory followUpDate lastFollowUpAt")
      .lean();

    // ---- Performance metrics ----
    const totalLeads = leadsInRange.length;
    const admissions = leadsInRange.filter((l) => l.status === ADMITTED_STATUS).length;
    const lostLeads = leadsInRange.filter((l) => LOST_STATUSES.includes(l.status)).length;
    const conversionRate = totalLeads > 0 ? +((admissions / totalLeads) * 100).toFixed(2) : 0;

    const callsDone = leadsInRange.filter((l) => l.followUpHistory?.length > 0).length;
    const followUps = leadsInRange.reduce((sum, l) => sum + (l.followUpHistory?.length || 0), 0);

    const responded = leadsInRange.filter((l) => l.firstResponseAt);
    const avgResponseMinutes =
      responded.length > 0
        ? Math.round(
            responded.reduce(
              (s, l) => s + (new Date(l.firstResponseAt) - new Date(l.createdAt)) / 60000,
              0
            ) / responded.length
          )
        : null;

    const resolved = leadsInRange.filter((l) => l.resolvedAt);
    const avgResolutionHours =
      resolved.length > 0
        ? +(
            resolved.reduce(
              (s, l) => s + (new Date(l.resolvedAt) - new Date(l.createdAt)) / 3600000,
              0
            ) / resolved.length
          ).toFixed(1)
        : null;
    ...
    // ============================================
    // ✅ PENDING CALLS — FULL POOL (no date filter)
    // ============================================
    // Definition: Open leads jinka follow-up due hai
    //   Case 1: followUpDate set hai aur aaj ya usse pehle ki hai
    //   Case 2: followUpDate set nahi, aur abhi tak koi follow-up nahi hua (fresh lead)
    const openStatusFilter = {
      $nin: [ADMITTED_STATUS, ...LOST_STATUSES, "Not Picked"],
    };

    const [pendingCalls, overdueCalls, freshUncontacted] = await Promise.all([
      // Total pending calls
      Lead.countDocuments({
        assignedTo: targetId,
        status: openStatusFilter,
        $or: [
          { followUpDate: { $lte: new Date(), $exists: true, $ne: null } },
          {
            $and: [
              { $or: [{ followUpDate: null }, { followUpDate: { $exists: false } }] },
              { $or: [{ lastFollowUpAt: null }, { lastFollowUpAt: { $exists: false } }] },
            ],
          },
        ],
      }),

      // Overdue = followUpDate <= aaj (khali overdue wale)
      Lead.countDocuments({
        assignedTo: targetId,
        status: openStatusFilter,
        followUpDate: { $lte: new Date(), $exists: true, $ne: null },
      ...
      // Fresh/uncontacted = koi follow-up nahi hua aur koi followUpDate bhi nahi
      Lead.countDocuments({
        assignedTo: targetId,
        status: openStatusFilter,
        $and: [
    ...
    res.status(200).json({
      ...
      data: {
        ...
        totalLeads,
        callsDone,
        pendingCalls,        // ✅ Full pool se
        overdueCalls,        // ✅ Naya
        freshUncontacted,    // ✅ Naya
        followUps,
        admissions,
        lostLeads,
        conversionRate,
...
/* ✅ UPDATED: getCounselorHoursSummary
   - IST-aware date boundaries (today, week, month)
*/
export const getCounselorHoursSummary = async (req, res) => {
  try {
    const targetId = req.params.counselorId || String(req.user._id);
